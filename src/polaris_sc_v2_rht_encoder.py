#!/usr/bin/env python3
"""Qwen real-weight adapter for the POLARIS-SC-v2 polar-lattice codec.
...
The polar core is intentionally kept byte-compatible with the frozen v2
encoder. This adapter adds only an optional decoder-visible orthonormal
randomized Hadamard preconditioner and bounded-memory BF16 block ingestion.
The preconditioner has no per-weight payload: its seed is derived by the
evaluation manifest from a global constant and the canonical block identity.
"""
...
def splitmix64_rademacher(n: int, seed: int) -> np.ndarray:
    """Return deterministic +/-1 signs from splitmix64(seed + index)."""
...
def normalized_hadamard_gpu(values: cp.ndarray) -> cp.ndarray:
    """Orthonormal Walsh-Hadamard transform, entirely on the GPU."""
    ...
    while width < out.size:
        ...
        view[:, 1, :] = left - right
        width *= 2
    out *= 1.0 / math.sqrt(int(out.size))
    return out
...
def run_trial(args: argparse.Namespace, trial: int, capacities: list[float], flags: list[np.ndarray]) -> dict:
    ...
    if args.input_bf16 is None:
        ...
        raw = np.memmap(args.input_bf16, dtype="<u2", mode="r")
        block_index = args.input_block_start + trial
        begin = block_index * n
        end = begin + n
        if end > raw.size:
            raise ValueError(
                f"input block {block_index} ends at {end}, beyond {raw.size} values"
            )
        raw_block = np.asarray(raw[begin:end], dtype=np.uint16).copy()
        values = (raw_block.astype(np.uint32) << np.uint32(16)).view(np.float32)
        block = cp.asarray(values, dtype=cp.float64)
        ...
        block_rms_pre_rht = block_rms
        rht_row: dict[str, object] = {"enabled": False}
        if args.apply_rht:
            rht_seed = (int(args.rht_seed) + 104729 * trial) & 0xFFFFFFFFFFFFFFFF
            signs = cp.asarray(splitmix64_rademacher(n, rht_seed))
            transformed = normalized_hadamard_gpu(block * signs)
            energy_before = float(cp.sum(block * block).get())
            energy_after = float(cp.sum(transformed * transformed).get())
            block = transformed
            block_rms = float(cp.sqrt(cp.mean(block * block)).get())
            rht_row = {
                "enabled": True,
                "mode": "hadamard_rademacher_splitmix64",
                "seed_u64": rht_seed,
                "normalization": "orthonormal",
                "forward": "H_N diag(signs) x / sqrt(N)",
                "inverse": "diag(signs) H_N y / sqrt(N)",
                "sign_rule": "splitmix64(seed_u64 + flat_index) low bit: 0 -> +1, 1 -> -1",
                "relative_energy_drift_fp64": (energy_after - energy_before) / energy_before,
                "rms_convention": "post_transform_fp64",
                "rms_pre_transform_fp64": block_rms_pre_rht,
                "rms_post_transform_fp64": block_rms,
            }
        ...
        source_row = {
            ...
            "canonical_block_index": (
                block_index if args.canonical_block_index is None
                else args.canonical_block_index + trial
            ),
            "values": n,
            "block_bf16_sha256": hashlib.sha256(
                raw_block.astype("<u2", copy=False).tobytes()
            ).hexdigest(),
...
def main() -> None:
    ...
    ap.add_argument("--canonical-source-id")
    ap.add_argument("--canonical-block-index", type=int)
    ap.add_argument("--apply-rht", action="store_true")
    ap.add_argument("--rht-seed", type=int, default=20260831)
