# CBIB-1 r3 local RTX 3060 one-use capability r3
...
This successor does not retry either consumed r1 or r2 authority. Its only runtime logic change accepts the first 16 UUID bytes from CuPy when the Windows ABI returns a longer buffer, records the raw length and trailing bytes, and still requires exact agreement with the independently pinned nvidia-smi UUID whenever CuPy reports a UUID. It uses fresh claim,
status, run, cache, authorization, and bridge identifiers. The CuPy cache is a
fresh child of the known-writable `C:\INT2__compression\tmp` directory. The
source-only test probes that exact parent through a separately named disposable
sibling, then proves every r2 production path remains absent.
