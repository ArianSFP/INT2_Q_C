...
AUTHORIZATION = "EXECUTE_CBIB1_R3_LOCAL3060_BRIDGE_ONCE_R3_6B20D57E"
...
RUN_ROOT = Path(r"C:\INT2__compression\tmp\cbib1_r3_local3060_qwen_once_20260903_r3_6b20d57e")
ATTEMPT_CLAIM = Path(
    r"C:\INT2__compression\tmp\CBIB1_R3_LOCAL3060_AUTHORITY_ATTEMPT_20260903_R3_6B20D57E.json"
)
...
CACHE_ROOT = Path(r"C:\INT2__compression\tmp\cbib1_r3_local3060_cupy_cache_20260903_r3_6b20d57e")
...
def canonical_uuid(raw) -> str:
    if isinstance(raw, str):
        ...
        packed = bytes(raw)
        need(len(packed) >= 16, "CUDA UUID bytes shorter than 16")
        text = packed[:16].hex()
    return "GPU-" + "-".join((text[:8], text[8:12], text[12:16], text[16:20], text[20:]))


def uuid_observation(raw, source: str) -> dict:
    if raw is None or raw == "":
        return {"canonical": None, "raw_length": 0, "reported": False,
                "source": "nvidia-smi-only", "trailing_hex": ""}
    if not isinstance(raw, str) and len(bytes(raw)) == 0:
        return {"canonical": None, "raw_length": 0, "reported": False,
                "source": "nvidia-smi-only", "trailing_hex": ""}
    if isinstance(raw, str):
        return {"canonical": canonical_uuid(raw), "raw_length": len(raw.encode("ascii")),
                "reported": True, "source": source, "trailing_hex": ""}
    packed = bytes(raw)
    return {"canonical": canonical_uuid(packed), "raw_length": len(packed),
            "reported": True, "source": source, "trailing_hex": packed[16:].hex()}


def validate_uuid_observation(raw, source: str) -> dict:
    observation = uuid_observation(raw, source)
    need(not observation["reported"] or observation["canonical"] == EXPECTED_DEVICE_UUID,
         "CuPy UUID disagrees with pinned nvidia-smi UUID")
    return observation
...
def validate_local_runtime():
    ...
    getter = getattr(runtime, "deviceGetUuid", None)
    if callable(getter):
        try:
            raw_uuid = getter(0)
            uuid_source = "cupy.runtime.deviceGetUuid"
        except Exception:
            raw_uuid = None
    if raw_uuid is None or raw_uuid == b"" or raw_uuid == "":
        candidate = props.get("uuid", props.get(b"uuid"))
        if candidate is not None and candidate != b"" and candidate != "":
            raw_uuid = candidate
            uuid_source = "cupy.runtime.getDeviceProperties.uuid"
    uuid = validate_uuid_observation(raw_uuid, uuid_source)
    need(str(name) == EXPECTED_DEVICE_NAME and
    ...
    return cp, {
        ...
        "cupy_uuid_canonical": uuid["canonical"],
        "cupy_uuid_raw_length": uuid["raw_length"],
        "cupy_uuid_reported": uuid["reported"],
        "cupy_uuid_source": uuid["source"],
        "cupy_uuid_trailing_hex": uuid["trailing_hex"],
        "nvidia_smi_uuid_crosscheck": EXPECTED_DEVICE_UUID,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.authorization != AUTHORIZATION:
        print(json.dumps({"schema": "cbib1-r3-local3060-bridge-hold-v0-r3",
                          "status": "HOLD_NO_PATH_RUNTIME_OR_PAYLOAD_ACCESS"},
                         sort_keys=True, separators=(",", ":")))
        return 2
    real_file(ATTEMPT_CLAIM)
    claim = json.loads(ATTEMPT_CLAIM.read_bytes())
    need(claim.get("schema") == "cbib1-r3-local3060-authority-attempt-claim-v0-r3" and
