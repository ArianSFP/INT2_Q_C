...
def main() -> int:
    ...
    need(authority.get("status") == "AUTHORIZED_NOT_EXECUTED" and
         ...
         authority["wrapper_sha256"] == sha(package / "run_authorized_local_qwen_once.py"),
         "authority pins")
    ...
    need(receipt.get("status") == "PASS_SOURCE_ONLY_NOT_EXECUTED" and
         receipt.get("payload_accessed") is False and receipt.get("gpu_initialized") is False and
         receipt.get("production_executed") is False, "source-only receipt")
    print(json.dumps({
        ...
        "schema": "cbib1-r3-local3060-capability-verification-v0-r1",
