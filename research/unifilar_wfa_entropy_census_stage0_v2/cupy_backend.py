...
class CuPyUnifilarBackend:
    ...
    def __init__(self, cp: Any):
        if sys.byteorder != "little":
            raise RuntimeError("UWFA CuPy backend requires little-endian host u16 input")
        self.cp = cp
    ...
    def pack_streams(self, streams: list[tuple[bytes, bytes, bytes]]) -> dict[str, Any]:
        ...
        h2d = sum(len(item) for item in bit_parts) + sum(len(item) for item in level_parts) + sum(len(item) for item in frequency_parts)
        h2d += 16 * len(streams)
    ...
    def subset(self, packed: dict[str, Any], indices: list[int]) -> dict[str, Any]:
        """Descriptor-only subset: payload arrays remain one cached GPU copy."""
        if not indices or any(type(index) is not int or not 0 <= index < int(packed["stream_count"]) for index in indices):
            raise ValueError("GPU subset indices")
        cp = self.cp
        index_array = cp.asarray(indices, dtype=cp.int64)
        self.stats["h2d_bytes"] += 8 * len(indices)
        return {
            "bits": packed["bits"],
            "levels": packed["levels"],
            "base_freq": packed["base_freq"],
            "offsets": packed["offsets"][index_array],
            "lengths": packed["lengths"][index_array],
            "stream_count": len(indices),
            "symbol_count": int(sum(int(packed["host_lengths"][index]) for index in indices)),
            "root_stream_indices": tuple(int(packed["root_stream_indices"][index]) for index in indices),
            "host_lengths": tuple(int(packed["host_lengths"][index]) for index in indices),
        }
    ...
    def fit_counts(self, packed: dict[str, Any], topology_id: int, states: int, reset_length: int) -> Any:
        ...
        self.stats["count_cell_symbol_updates"] += int(packed["symbol_count"])
        return counts
    ...
    def exact_lengths(
        ...
        cp.cuda.runtime.deviceSynchronize()
        self.stats["kernel_count"] += 1
        self.stats["length_cell_symbol_updates"] += int(packed["symbol_count"])
        return result
    ...
    def environment_receipt(self) -> dict[str, Any]:
        ...
        used, total = cp.cuda.runtime.memGetInfo()
        return {
            ...
            "explicit_device_synchronization_after_every_kernel": True,
        }
