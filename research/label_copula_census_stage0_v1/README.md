...
and Gaussian controls passed through the complete frozen STRATA encoder.  v1
...
Layer and expert labels are permitted only as partition metadata.  The panel
must contain exactly one stream per layer/slot and the identical reusable
expert-slot universe in every layer.  Irregular panels and layer-unique
pseudo-slots fail closed.

- At least ten whole layers are required.  Outer test layers are selected by a
  frozen hash order, with at least five whole-layer test clusters.  Every expert in
...
cluster-bootstrap replicates, resampling selected/factorized layer deltas
together and allocating the exact shared-byte delta by layer source weight.
Fewer than five test-layer clusters cannot produce a confidence claim.  The
lower endpoint of the two-sided 95% interval—not merely the point estimate—must exceed
...
Eight seeds are frozen.  The evaluator accepts the immutable raw SwiGLU panel,
not caller-supplied control symbol streams.  For raw view A, every control must
independently:
...
2. rerun canonical orientation, RMS normalization, Lloyd labeling, and Gray
   serialization;
3. rerun the complete nested train/validation/test model search over all 240
   nonlocal cells and all eight factorized resets;
4. refit its independently selected source model;
5. arithmetic-code and charge its own physical packets and reads;
6. bind the seed, source tensor, source geometry, exact binary64 block moments,
   generated tensor, generated labels, bank and complete result with SHA-256.
...
For one routed expert, the read ledger records the literal union of header,
selected-model, addressed-directory-entry and intersecting-frame page indices.
Its denominator is `B_shared/E + B_private_i`, with final container padding in
`B_shared`; all expert denominators sum exactly to the physical container.  The
report includes maximum cold-read amplification.  This diagnostic ledger is
not substituted for the final MoE
...
is imported only after those gates; future GPU optimization is CuPy-only.  v1
then emits a preflight receipt with zero payload opens and zero kernels.
`COMPLETE.json` is created exclusively last.  Completion irreversibly closes
the writer: later members and a second completion are rejected.  The package
path and every ancestor are checked lexically for symlinks or Windows reparse
points before resolution or member opening.  An incomplete directory is never
resumed or accepted.

This package has **no payload authority and no claim authority**.  Its public
token, self-manifest, and producer-side checks are integrity mechanisms, not
authorization.  A future payload runner must be authenticated and executed by
an independently pinned bootstrap outside this producer package.  The v1
entrypoint only performs a source/CuPy availability preflight and cannot open a
checkpoint, current-codec stream, or Gaussian-control payload.

The launch gate is intentionally not a payload runner:

```bash
python3 -B -I research/label_copula_census_stage0_v1/stage0_census.py \
