#!/usr/bin/env python3
"""Hostile source-only UWFA-SC v5 producer tests.
...
common = load("uwfa_v5_test_common", "uwfa_common.py")
protocol = load("uwfa_v5_test_protocol", "protocol.py")
semantic = load("uwfa_v5_test_semantic", "universal_adapter.py")
codec = load("uwfa_v5_test_container", "container_codec.py")
fixture = load("uwfa_v5_test_fixture", "fixture_portability.py")
strata = load("uwfa_v5_test_strata", "strata_sc_adapter.py")
stage = load("uwfa_v5_test_stage", "stage0_census.py")
cupy_backend = load("uwfa_v5_test_cupy", "cupy_backend.py")
dispatcher = load("uwfa_v5_test_dispatcher", "dispatcher_contract.py")
envelope = load("uwfa_v5_test_envelope", "result_envelope.py")
...
def _valid_source_preflight_payload() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], str]:
    ...
    pci_bus_id = "00000000:16:00.0"
    independent = {
        "schema": "uwfa-sc-v5-independent-gpu-identity",
...
def build_fixture(experts: int = 2, hidden: int = 3, intermediate: int = 5, *, shared: bool = True) -> tuple[bytes, dict[str, Any], dict[str, Any]]:
    ...
    raw, metrics = codec.build_container(
        ...
        immutable_state=b"source-only-v5-fixture",
...
class PortabilityTests(unittest.TestCase):
    ...
    def test_e250_unequal_shape_shared_tail_descriptor_portability(self) -> None:
        ...
        raw, _diagnostic = codec.build_container(
            ...
            immutable_state=b"e250-unequal-v5", regions=source["regions"],
...
class ScientificPipelineTests(unittest.TestCase):
    ...
    def test_score_receipt_binds_full_geometry_and_independent_decoder(self) -> None:
        ...
        record = {
            "schema": "uwfa-bound-baseline-score-v5",
