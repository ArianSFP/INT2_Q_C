# Universal UWFA-SC v5 — pre-review source-only producer

This directory is a new, source-only successor to the independently blocked v4
producer. The v4 tree and its audit remain unchanged. This v5 tree is
...
The only measured object is one complete `UWFCV5` byte string:
...
Publication is likewise fail-closed at the last output boundary. Immediately
before constructing `COMPLETE.json`, the producer re-enumerates the retained
staging-directory descriptor and no-follow reopens, fstats, and rehashes every
declared regular member. It repeats the exact-membership/hash check including
`COMPLETE.json` immediately before the no-replace rename. A changed declared
member or any undeclared staging member aborts without publishing a final
directory; the retained-parent, symlink rejection, no-replace, and irrevocable
post-rename properties remain unchanged.
...
- an externally pinned v5 source audit;
