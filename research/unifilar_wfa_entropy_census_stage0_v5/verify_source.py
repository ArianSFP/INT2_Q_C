#!/usr/bin/env python3
"""Native verifier for a post-review sealed UWFA-SC v5 source package."""
...
POST_FREEZE_REQUIREMENTS = [
    "EXTERNAL_PINNED_V5_SOURCE_AUDIT",
...
def verify_package(package: Path) -> dict[str, Any]:
    ...
    require(manifest["schema"] == "unifilar-wfa-source-manifest-v5", "manifest schema")
    ...
    require(design.get("schema") == "unifilar-wfa-entropy-census-design-v5", "design schema")
    ...
    common = module_from_snapshot("uwfa_verify_common_v5", snapshots["uwfa_common.py"])
    ...
    require(model.startswith(b"UWFAV5\x00\x00"), "v5 serialized model")
    ...
    for token in (
        ...
        require(token in stage_text, f"v5 source gate: {token}")
    ...
    require(common_text.count("self._verify_exact_staging_members(") == 2, "staging verification immediately before COMPLETE and rename")
    ...
    for token in (
        ...
        require(token in tests_text, f"V5 hostile regression: {token}")

    return {
        "schema": "unifilar-wfa-source-verification-v5",
