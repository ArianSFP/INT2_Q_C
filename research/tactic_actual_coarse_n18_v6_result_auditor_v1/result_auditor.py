#!/usr/bin/env python3
"""Fail-closed successor audit of one real N18-v6 Qwen pilot result.

This v1 package is a minimal post-failure schema repair: v0 omitted the
producer's exact-input-byte scoring attestation from its expected mapping.
No producer artifact is changed.  A run still needs an independently hashed
external pin bundle.  All executable and data bytes are retained and
authenticated before NumPy or the separately pinned polar decoder is loaded.
The producer's RESULT remains comparison-only, never a numerical input.
"""
...
AUDIT_SCHEMA = "tactic-actual-coarse-n18-v6-qwen-independent-result-audit-v1"
OWN_MANIFEST_SCHEMA = "tactic-actual-coarse-n18-v6-result-auditor-source-manifest-v1"
OWN_MANIFEST_STATUS = "SEALED_POST_FAILURE_SCHEMA_REPAIR_AWAITING_RERUN"
OWN_ROOT_DOMAIN = b"TACTIC-ACTUAL-COARSE-N18-V6-RESULT-AUDITOR-SOURCE-ROOT-V1\0"
