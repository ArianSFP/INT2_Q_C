...
def producer_score_view(recomputed_score: Mapping[str, Any]) -> dict[str, Any]:
    """Return the exact v6 producer-facing score schema.

    The independent scorer additionally computes raw MSE and explicit weight
    counts.  The authenticated v6 producer receipts intentionally omit those
    redundant fields and instead attest that scoring used the exact encoder
    input bytes.  Keep that attestation in the expected mapping: dropping it
    makes an otherwise valid receipt fail the exact-field comparison, which
    was the defect in result-auditor v0.
    """

    require(
        set(recomputed_score) == {
            "domain",
            "roles",
            "pooled_sse_fp64",
            "pooled_source_energy_fp64",
            "pooled_raw_mse_fp64",
            "pooled_relative_mse",
            "zero_pooled_source_energy",
        },
        "independent score fields",
    )
    roles = recomputed_score["roles"]
    require(isinstance(roles, list), "independent score roles")
    expected_roles = []
    for ordinal, role in enumerate(roles):
        require(
            ...
            f"independent score role {ordinal} fields",
        )
        expected_roles.append(
            {
                key: value
                for key, value in role.items()
                if key not in {"weights", "raw_mse_fp64"}
            }
        )
    expected = {
        key: value
        for key, value in recomputed_score.items()
        if key not in {"roles", "pooled_raw_mse_fp64"}
    }
    expected["roles"] = expected_roles
    expected["scorer_uses_exact_encoder_input_bytes"] = True
    return expected
...
def validate_result_material(
    ...
    *, expected_status: str, source_root_sha256: str, smoke_binding: Mapping[str, Any], runtime_receipt: Mapping[str, Any],
    ...
    expected_score = producer_score_view(recomputed["original_domain_score"])
