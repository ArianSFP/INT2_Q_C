...
class ProducerScoreSchemaTests(unittest.TestCase):
    def independent_score(self) -> dict:
        roles = []
        for ordinal, role in enumerate(core.ROLES):
            roles.append(
                {
                    "role": role,
                    "weights": 8,
                    "source_bf16_sha256": f"{ordinal + 1:064x}",
                    "reconstruction_f32_sha256": f"{ordinal + 4:064x}",
                    "residual_f64_sha256": f"{ordinal + 7:064x}",
                    "sse_fp64": float(ordinal + 1),
                    "source_energy_fp64": float(ordinal + 11),
                    "raw_mse_fp64": float(ordinal + 1) / 8.0,
                    "relative_mse": float(ordinal + 1) / float(ordinal + 11),
                    "zero_source_energy": False,
                }
            )
        return {
            ...
            "roles": roles,
            "pooled_sse_fp64": 6.0,
            "pooled_source_energy_fp64": 36.0,
            "pooled_raw_mse_fp64": 0.25,
            "pooled_relative_mse": 1.0 / 6.0,
            "zero_pooled_source_energy": False,
        }

    def test_exact_producer_view_keeps_input_byte_attestation(self) -> None:
        expected = core.producer_score_view(self.independent_score())
        self.assertTrue(expected["scorer_uses_exact_encoder_input_bytes"])
        self.assertNotIn("pooled_raw_mse_fp64", expected)
        self.assertNotIn("weights", expected["roles"][0])
        self.assertNotIn("raw_mse_fp64", expected["roles"][0])
        core.require_deep_close(copy.deepcopy(expected), expected, "producer score")

    def test_missing_input_byte_attestation_rejected(self) -> None:
        expected = core.producer_score_view(self.independent_score())
        hostile = copy.deepcopy(expected)
        hostile.pop("scorer_uses_exact_encoder_input_bytes")
        with self.assertRaises(core.AuditError):
            core.require_deep_close(hostile, expected, "producer score")

    def test_false_input_byte_attestation_rejected(self) -> None:
        expected = core.producer_score_view(self.independent_score())
        hostile = copy.deepcopy(expected)
        hostile["scorer_uses_exact_encoder_input_bytes"] = False
        with self.assertRaises(core.AuditError):
            core.require_deep_close(hostile, expected, "producer score")

    def test_extra_producer_score_field_rejected(self) -> None:
        expected = core.producer_score_view(self.independent_score())
        hostile = copy.deepcopy(expected)
        hostile["unbound"] = True
        with self.assertRaises(core.AuditError):
            core.require_deep_close(hostile, expected, "producer score")

    def test_unexpected_independent_score_field_rejected(self) -> None:
        hostile = self.independent_score()
        hostile["unbound"] = True
        with self.assertRaises(core.AuditError):
            core.producer_score_view(hostile)
