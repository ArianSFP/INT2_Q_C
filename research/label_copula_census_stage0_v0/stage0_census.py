#!/usr/bin/env python3
"""Fail-closed launch gate for a future authenticated raw-label census.

This sealed v0 package has no payload authority.  It validates lifecycle order
...
_COMMON_PATH = Path(__file__).resolve().with_name("label_copula_common.py")
...
def _dynamic_verify_source() -> dict[str, object]:
    ...
    return module.verify_package(Path(__file__).resolve().parent)


def _review(path: Path, manifest_sha256: str) -> dict[str, object]:
...
def _input_metadata(path: Path) -> dict[str, object]:
    ...
    require(record.get("stream_view") == "A_raw_normalized_gaussian_lloyd4", "v0 raw-label-only view")
...
def main() -> int:
    ...
    try:
        with CompletionLastOutput(args.output.resolve()) as output:
            source = _dynamic_verify_source()
            manifest_sha256 = str(source["manifest_sha256"])
            review = _review(args.review_receipt.resolve(), manifest_sha256)
            metadata = _input_metadata(args.input_lock.resolve())

            # CuPy is intentionally late.  Future GPU work is CuPy-only; this
            # v0 preflight does not open any checkpoint or initialize a payload
            # adapter.  Import success is not a scientific result.
            import cupy as cp

            preflight = {
                "schema": "label-copula-census-cupy-preflight-v0",
                "status": "PASS_CUPY_AVAILABLE_NO_PAYLOAD_OPENED",
                ...
                "claim_boundary": "Lifecycle/CuPy preflight only; not a label-census result.",
            }
