#!/usr/bin/env python3
"""Authenticated-snapshot producer core for universal UWFA-SC v3.
...
PRODUCER_ABI = "uwfa-sc-v3-authenticated-snapshot-producer-abi-1"
...
def representative_outer_fold_benchmark(
    ...
    semantic_packet = semantic_codec.build_semantic_packet(shapes, b"representative-outer-fold-v3")
    ...
    development = [index for index, row in enumerate(rows) if all(owner not in {0, 1} for owner in row["owners"])]
    ...
    for row in rows:
        ...
        stream_specs.append((
            container_codec.StreamSpec(
                ...
                payload=payload,
                source_digest=hashlib.sha256(bytes(row["bits"])).hexdigest(),
                profile_q=0,
                decoder_scale=1.0,
                role=str(row["role"]),
...
def _split_digest(protocol: Any, identity: tuple[int, int], row: Mapping[str, Any]) -> bytes:
    digest = protocol.length_prefixed_digest(
        ...
        domain=b"UWFA-SC-V3-NESTED-SPLIT-2026-09-02\x00",
    )
        ...
        for context, positions in groups.items():
            ranked_sources = sorted(
                ...
                key=lambda position: protocol.length_prefixed_digest(
                    ...
                    domain=b"UWFA-SC-V3-WITHIN-CONTEXT-PERMUTATION\x00",
                ),
            )
    ...
    for row in panel["streams"]:
        ...
        ranked = sorted(
            ...
            key=lambda item: protocol.length_prefixed_digest(
                ...
                domain=b"UWFA-SC-V3-MULTISCALE-CHUNK-SHUFFLE\x00",
            ),
        )
...
def source_phase(
    *,
    ...
    elapsed = time.perf_counter() - started
    result = {
        "schema": "uwfa-sc-v3-source-phase-result",
        "status": status,
...
def controls_phase(
    *,
    ...
    for expected_seed, item in zip(common.CONTROL_SEEDS, controls, strict=True):
        ...
        if evidence.pipeline_sha256 != source_pipeline or evidence.source_panel_sha256 != geometry:
    ...
    return {
        "schema": "uwfa-sc-v3-control-phase-result",
