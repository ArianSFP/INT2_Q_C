# UWFA-SC v3 independent bootstrap ABI
...
- the SHA-256 of an independent v3 source-review receipt whose schema is
  `unifilar-wfa-entropy-census-independent-source-review-v3`;
...
- `container_codec.py`: literal `UWFCV3` build/parse/rebuild/routed-read ledger;
...
3. Execute source-free all-150 CPU/CuPy equality and the 15-stream/6-owner
   ...
   time, process-tree RSS/HWM, VRAM/pool peaks, runtime/driver/device.
...
6. Run every source outer fold over all 150 cells, winner refit, exact literal
   ...
   reads.
7. Only if every source gate passes, authenticate all eight controls before any
   control fit, then repeat the entire 150-cell pipeline independently for each.
8. Write outputs exclusively through a retained output-directory descriptor;
   fsync each member, write `COMPLETE.json` exclusively and last, fsync the
   directory, and do not resume an incomplete directory.
...
The literal result also carries a `uwfa-sc-v3-posterior-diagnostic-handoff`
