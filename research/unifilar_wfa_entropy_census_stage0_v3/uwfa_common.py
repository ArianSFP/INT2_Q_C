#!/usr/bin/env python3
"""Standard-library contracts and exact CPU reference for UWFA census v3.
...
DESIGN_SCHEMA = "unifilar-wfa-entropy-census-design-v3"
RESULT_SCHEMA = "unifilar-wfa-entropy-census-result-v3"
REVIEW_SCHEMA = "unifilar-wfa-entropy-census-independent-source-review-v3"
STREAM_LOCK_SCHEMA = "unifilar-wfa-canonical-stream-lock-v3"
CONTROL_LOCK_SCHEMA = "unifilar-wfa-gaussian-control-panel-lock-v3"
AUTHORIZATION = "OPEN_AUTHENTICATED_UNIFILAR_WFA_CENSUS_AFTER_INDEPENDENT_SOURCE_REVIEW_V3"
...
SPLIT_HASH_SEED = b"UWFA-V3-NESTED-SPLIT-2026-09-02"
...
def deserialize_model(packet: bytes) -> tuple[Candidate, list[int]]:
    ...
    require(magic == b"UWFAV3\x00\x00" and version == 3, "model magic/version")
...
class CompletionLastOutput:
    """Descriptor-relative, durable, completion-last output transaction."""
    ...
    def __enter__(self) -> "CompletionLastOutput":
        _raw_absolute_parts(self.path)
        require(not os.path.lexists(self.path), "output path already exists")
        os.mkdir(self.path, 0o700)
        try:
            nofollow = getattr(os, "O_NOFOLLOW", 0)
            directory = getattr(os, "O_DIRECTORY", 0)
            self.dir_fd = os.open(str(self.path), os.O_RDONLY | directory | nofollow)
            require(stat.S_ISDIR(os.fstat(self.dir_fd).st_mode), "output descriptor is not a directory")
            self.write_new("RUN_STATE.json", pretty_json({"schema": "unifilar-wfa-run-state-v3", "complete": False}))
            return self
        except Exception:
            if self.dir_fd is not None:
                os.close(self.dir_fd)
                self.dir_fd = None
            # Removing the exact just-created directory is attempted only while
            # it is empty. rmdir cannot remove RUN_STATE or any later member;
            # once a durable member exists every fault remains visibly
            # incomplete and fail-closed for independent inspection.
            try:
                os.rmdir(self.path)
            except OSError:
                pass
    ...
    def write_new(self, name: str, data: bytes) -> dict[str, Any]:
        ...
        if os.name != "nt" and os.open in os.supports_dir_fd:
            ...
            # Windows source-only fallback; numeric evidence requires the Unix
            # descriptor-relative branch and records that fact.
            fd = os.open(str(self.path / name), flags, 0o600)
        try:
    ...
    def complete(self, members: list[dict[str, Any]], source_manifest_sha256: str) -> dict[str, Any]:
        ...
        record = seal_record(
            {
                "schema": "unifilar-wfa-completion-v3",
