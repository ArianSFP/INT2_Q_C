#!/usr/bin/env python3
"""Native verifier for a post-review sealed UWFA-SC v3 source package."""
...
POST_FREEZE_REQUIREMENTS = [
    "EXTERNAL_PINNED_V3_SOURCE_AUDIT",
...
def verify_package(package: Path) -> dict[str, Any]:
    ...
    require(manifest["schema"] == "unifilar-wfa-source-manifest-v3", "manifest schema")
    ...
    require(design.get("schema") == "unifilar-wfa-entropy-census-design-v3", "design schema")
    ...
    common = module_from_snapshot("uwfa_verify_common_v3", snapshots["uwfa_common.py"])
    ...
    require(model.startswith(b"UWFAV3\x00\x00"), "v3 serialized model")
    ...
    for token in ("UWFCV3", "DIRECTORY_RECORD_BYTES = 256", "REGION_HEADER_BYTES = 256", "FRAME_HEADER_BYTES = 256", "def routed_read_expert", "def physical_metrics", "def posterior_diagnostic_handoff"):
    ...
    for token in ("h2d_model_table_bytes", "h2d_launch_descriptor_bytes", "h2d_kernel_scalar_bytes", "d2h_bytes", "peak_process_tree_rss_bytes", "peak_vram_incremental_bytes", "peak_pinned_pool_free_blocks"):
        require(token in cuda_text, f"telemetry contract: {token}")

    return {
        "schema": "unifilar-wfa-source-verification-v3",
