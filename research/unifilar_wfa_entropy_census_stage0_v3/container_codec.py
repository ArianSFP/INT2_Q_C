#!/usr/bin/env python3
"""Canonical UWFA-SC v3 literal container and routed-read ledger.

The v3 ABI uses one fixed 32-byte little-endian owner set everywhere. It
...
MAGIC = b"UWFCV3\x00\x00"
DIRECTORY_MAGIC = b"UWFDIR3\x00"
REGION_MAGIC = b"UWFREG3\x00"
FRAME_MAGIC = b"UWFFRM3\x00"
VERSION = 3
HEADER_BYTES = 4096
...
def _header(*, weights: int, experts: int, streams: int, regions: int, baseline_object_bytes: int, audited_relative_mse: float, semantic_offset: int, semantic_bytes: int, immutable_offset: int, immutable_bytes: int, model_offset: int, model_bytes: int, directory_offset: int, directory_bytes: int, shared_bytes: int, total_bytes: int, rate_num: int, rate_den: int, baseline_artifact_sha256: str, reconstruction_sha256: str, audit_binding_sha256: str, semantic_sha: bytes, immutable_sha: bytes, model_sha: bytes, directory_sha: bytes, body_sha: bytes, binding_hashes: Mapping[str, str]) -> bytes:
    ...
    if set(binding_hashes) != set(_HEADER_BINDINGS):
        raise ValueError("exact v3 container binding fields required")
...
def build_container(
    ...
    *,
    ...
    directory_bytes = _checked_mul(len(flat), DIRECTORY_RECORD_BYTES, "directory bytes")
    semantic_offset = HEADER_BYTES
    immutable_offset = _align(_checked_add(semantic_offset, len(semantic_packet), "semantic end"), 64)
    model_offset = _align(_checked_add(immutable_offset, len(immutable_state), "immutable end"), PAGE_BYTES)
    directory_offset = _align(_checked_add(model_offset, len(model_packet), "model end"), PAGE_BYTES)
    shared_bytes = _align(_checked_add(directory_offset, directory_bytes, "directory end"), PAGE_BYTES)
    region_lengths = [_align(_checked_add(REGION_HEADER_BYTES, len(area), "region content"), PAGE_BYTES) for area in frame_areas]
    current_total = shared_bytes
    for length in region_lengths:
        current_total = _checked_add(current_total, length, "container region sum")
    rate_bits_numerator = _checked_mul(weights, rate_num, "rate-floor weight product", MAX_WEIGHTS * 10_000)
    minimum_total = _ceil_div(rate_bits_numerator, 8 * rate_den, "rate-floor bytes")
    if minimum_total > MAX_CONTAINER_BYTES:
        raise ValueError("rate-floor container exceeds frozen byte bound")
    required_padding_bytes = max(0, minimum_total - current_total)
    padding_pages = _ceil_div(required_padding_bytes, PAGE_BYTES, "rate-floor padding pages")
    ...
    for index in range(len(region_lengths)):
        ...
        extra = _checked_mul(pages, PAGE_BYTES, "rate-floor region padding", MAX_CONTAINER_BYTES)
        region_lengths[index] = _checked_add(region_lengths[index], extra, "rate-floor region pad")
    current_total = _checked_add(current_total, _checked_mul(padding_pages, PAGE_BYTES, "rate-floor total padding", MAX_CONTAINER_BYTES), "rate-floor total pad")
...
def posterior_diagnostic_handoff(common: Any, parsed: Mapping[str, Any]) -> dict[str, Any]:
    ...
    public-context state, semantic routes, per-stream decoded-bit commitments,
    ...
    for ordinal, row in enumerate(parsed["directory"]):
        ...
        decision_rows.append({
            ...
            "decoded_symbol_bits_sha256": str(row["source_digest"]),
    ...
    return {
        "schema": "uwfa-sc-v3-posterior-diagnostic-handoff",
        ...
        "source_panel_sha256": str(parsed["binding_hashes"]["source_panel_sha256"]),
        ...
        "decoded_sc_decision_commitment_sha256": commitment,
