#!/usr/bin/env python3
"""Hostile source-only regressions for replay-safe BMP/OBDD/QTT6 v2."""
...
from production_hooks import (AUDIT_SCHEMA, CONTROL_SCHEMA, READ_SCHEMA,
                              ArtifactBinding, ProductionHooks,
                              held_source_only_hooks)
from search import (CompleteRateCap, candidate_serialized_capacity,
                    exact_workspace_plan, exhaustive_small_indices)
...
class HardenedSourceOnlyTests(unittest.TestCase):
    ...
    def test_exact_workspace_named_ledger(self):
        ...
        self.assertEqual(dict(plan)["stable_order_intp_capacity"],
                         4096 * np.dtype(np.intp).itemsize)
        self.assertNotIn("stable_order_i32", dict(plan))
        ledger = search.WorkspaceLedger(cap_bytes=10)
        ledger.own("x", 10, dtype="bytes", shape=(10,))
        with self.assertRaisesRegex(CodecError, "workspace cap"):
            ledger.own("y", 1, dtype="bytes", shape=(1,))

    def test_skew_geometry_packet_capacities_are_dimension_derived(self):
        skew = Geometry(4096, 4096, 0, 0, 1, 0, 4096)
        ledger = candidate_serialized_capacity(skew)
        rank_one = next(row for row in ledger["candidates"]
        ...
        self.assertEqual(rank_one["maximum_packet_bytes"], expected)
        self.assertGreater(expected, 2048)
        self.assertEqual(ledger["candidate_count"], 16)

    def test_runtime_owned_object_ledger_charges_actual_intp_and_packets(self):
        table = np.zeros((4096, 64), np.float64)
        # A full QTT bank is intentionally expensive; patch its candidates out
        # while exercising the real BMP stable-order and packet ownership path.
    ...
    def test_production_hooks_fail_closed(self):
        with self.assertRaisesRegex(CodecError, "unbound production objects"):
            held_source_only_hooks().authorize()

    def test_production_hooks_authenticate_objects_and_semantic_receipts(self):
        def dump(path: Path, value) -> ArtifactBinding:
            payload = (json.dumps(value, sort_keys=True,
                                  separators=(",", ":")).encode("utf-8")
                       if isinstance(value, dict) else bytes(value))
            path.write_bytes(payload)
            return ArtifactBinding(path, hashlib.sha256(payload).hexdigest())

        source_manifest = "1" * 64
        source_root = "2" * 64
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            binary = {name: dump(root / name, name.encode("ascii")) for name in (
                "strata", "scale", "forward", "inverse", "scorer",
                "factory", "framer")}
            read = dump(root / "read.json", {
                "schema": READ_SCHEMA,
                "maximum_routed_read_amplification": 1.25})
            audit = dump(root / "audit.json", {
                "schema": AUDIT_SCHEMA, "passed": True,
                "producer_source_manifest_sha256": source_manifest,
                "producer_source_root_sha256": source_root})
            controls = tuple(dump(root / f"control-{index}.json", {
                "schema": CONTROL_SCHEMA,
                "identical_complete_selection_replayed": True,
                "selected_control": True,
                "control_id": f"control-{index}"}) for index in range(8))
            hooks = ProductionHooks(
                strata_packet=binary["strata"], scale_decoder=binary["scale"],
                forward_transform=binary["forward"],
                inverse_transform=binary["inverse"],
                original_bf16_scorer=binary["scorer"],
                gaussian_control_factory=binary["factory"],
                component_framer=binary["framer"], routed_read_ledger=read,
            ...
            # A syntactically valid but false digest must not authorize.
            bad = ArtifactBinding(binary["strata"].path, "0" * 64)
            with self.assertRaisesRegex(CodecError, "object SHA-256"):
                ProductionHooks(**{**hooks.__dict__, "strata_packet": bad}).authorize()

    def test_manifest_self_replay_cli_exactly_as_documented(self):
        ...
        self.assertEqual(record["source_manifest_sha256"], digest)
        self.assertEqual(record["status"],
                         "PASS_EXACT_SOURCE_INVENTORY__RUNTIME_PENDING__HOLD_PAYLOAD")
