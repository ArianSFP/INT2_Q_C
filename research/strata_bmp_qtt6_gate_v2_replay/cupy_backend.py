...
def search_bmp_rank01_cupy(distortion, geometry, lambda_bit: float, rate_cap):
    ...
    return {
        ...
        "workspace": {
            "accounting": "logical_capacity_separate_from_measured_cupy_pool",
            "logical_serialized_capacity": candidate_serialized_capacity(
                geometry),
            "actual_retained_candidate_packet_bytes": sum(
                len(row["packet"]) for row in candidates),
            "measured_cupy_pool": {
            ...
            "cross_allocator_peak_claimed": False,
        },
