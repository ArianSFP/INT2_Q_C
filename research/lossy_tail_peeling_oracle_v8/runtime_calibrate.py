#!/usr/bin/env python3
"""Source-free v8 runtime calibration entrypoint.
...
def fail(message: str) -> "NoReturn":  # type: ignore[name-defined]
    raise SystemExit(f"V8_RUNTIME_CALIBRATE_REJECT: {message}")
...
def main() -> None:
    ...
    if contract.get("schema") != "lossy-tail-v8-runtime-calibration-contract-v1" or contract.get("status") != "FROZEN_SOURCE_FREE_BEFORE_RUNTIME_CALIBRATION":
        fail("runtime contract status mismatch")

    oracle_module = types.ModuleType("lossy_tail_v8_runtime_core")
    oracle_module.__file__ = os.fspath(stage / "lossy_tail_core.py")
    oracle_module.__package__ = None
    oracle_module.__dict__["__V8_CORE_CONTEXT__"] = {
