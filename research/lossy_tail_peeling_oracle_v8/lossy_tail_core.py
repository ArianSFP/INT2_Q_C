#!/usr/bin/env python3
"""NumPy/CuPy scientific core for lossy-tail peeling v8.
...
import hashlib as _v8_firewall_hashlib
import os as _v8_firewall_os
import stat as _v8_firewall_stat
import sys as _v8_firewall_sys

try:
    import fcntl as _v8_firewall_fcntl  # type: ignore
except ImportError:  # Windows source-test imports only; production is Linux-only.
    _v8_firewall_fcntl = None  # type: ignore


_V8_CORE_CONTEXT = globals().get("__V8_CORE_CONTEXT__")
if (
    not isinstance(_V8_CORE_CONTEXT, dict)
    or _V8_CORE_CONTEXT.get("schema") != "lossy-tail-v8-core-context-v1"
    or _V8_CORE_CONTEXT.get("mode") not in {"production_child", "runtime_calibration", "source_cpu_test"}
    or __name__ == "__main__"
):
    raise RuntimeError("V8_CORE_FIREWALL_REJECT: authenticated descriptor context required before NumPy import")


def _v8_preimport_production_firewall(context: dict) -> None:
    """Reprove immutable preflight/output descriptors before NumPy import."""
    ...
    if set(context) != expected_context_keys:
        raise RuntimeError("V8_CORE_FIREWALL_REJECT: production context key drift before NumPy import")
    if context.get("child_pid") != _v8_firewall_os.getpid() or context.get("parent_pid") != _v8_firewall_os.getppid():
        raise RuntimeError("V8_CORE_FIREWALL_REJECT: production process identity mismatch before NumPy import")
    flags = [
    ...
    raw = _v8_firewall_sys.argv[1:]
    if len(raw) != 2 * len(flags) or raw[::2] != flags or raw[19] != "4" or raw[21] != "4":
        raise RuntimeError("V8_CORE_FIREWALL_REJECT: production grammar mismatch before NumPy import")
    bootstrap = _v8_firewall_sys.argv[0]
    core_file = globals().get("__file__")
    if (
        not isinstance(bootstrap, str) or not isinstance(core_file, str)
        or not _v8_firewall_os.path.isabs(bootstrap) or not _v8_firewall_os.path.isabs(core_file)
        or bootstrap != _v8_firewall_os.path.normpath(bootstrap)
        or core_file != _v8_firewall_os.path.normpath(core_file)
        or bootstrap != _v8_firewall_os.path.realpath(bootstrap)
        or core_file != _v8_firewall_os.path.realpath(core_file)
        or _v8_firewall_os.path.basename(bootstrap) != "lossy_tail_oracle.py"
        or _v8_firewall_os.path.basename(core_file) != "lossy_tail_core.py"
        or _v8_firewall_os.path.dirname(bootstrap) != _v8_firewall_os.path.dirname(core_file)
    ):
        raise RuntimeError("V8_CORE_FIREWALL_REJECT: raw bootstrap/core stage mismatch before NumPy import")
    stage = _v8_firewall_os.path.dirname(core_file)
    exact_stage_values = {
    ...
    if any(raw[index] != _v8_firewall_os.path.join(stage, name) for index, name in exact_stage_values.items()):
        raise RuntimeError("V8_CORE_FIREWALL_REJECT: stage argument mismatch before NumPy import")
    if _v8_firewall_fcntl is None:
        raise RuntimeError("V8_CORE_FIREWALL_REJECT: Linux descriptor seals required before NumPy import")
    try:
        preflight_fd = int(context["preflight_memfd_fd"])
        output_fd = int(context["output_parent_fd"])
        preflight_stat = _v8_firewall_os.fstat(preflight_fd)
        parent_preflight_stat = _v8_firewall_os.stat(f"/proc/{context['parent_pid']}/fd/{preflight_fd}")
        output_stat = _v8_firewall_os.fstat(output_fd)
        parent_output_stat = _v8_firewall_os.stat(f"/proc/{context['parent_pid']}/fd/{output_fd}")
        seals = int(_v8_firewall_fcntl.fcntl(preflight_fd, _v8_firewall_fcntl.F_GET_SEALS))
        required_seals = sum(int(getattr(_v8_firewall_fcntl, name)) for name in (
            "F_SEAL_SEAL", "F_SEAL_SHRINK", "F_SEAL_GROW", "F_SEAL_WRITE",
        ))
        preflight_payload = _v8_firewall_os.pread(preflight_fd, preflight_stat.st_size, 0)
        stage_preflight_fd = _v8_firewall_os.open(
            _v8_firewall_os.path.join(stage, "preflight_launch.py"),
            _v8_firewall_os.O_RDONLY | getattr(_v8_firewall_os, "O_CLOEXEC", 0)
            | getattr(_v8_firewall_os, "O_NOFOLLOW", 0),
        )
        try:
            stage_preflight_stat = _v8_firewall_os.fstat(stage_preflight_fd)
            stage_preflight_payload = _v8_firewall_os.pread(stage_preflight_fd, stage_preflight_stat.st_size, 0)
        finally:
            _v8_firewall_os.close(stage_preflight_fd)
    except (OSError, KeyError, TypeError) as exc:
        raise RuntimeError("V8_CORE_FIREWALL_REJECT: cannot authenticate inherited descriptors before NumPy import") from exc
    preflight_hash = _v8_firewall_hashlib.sha256(preflight_payload).hexdigest()
    if (
        not _v8_firewall_stat.S_ISREG(preflight_stat.st_mode)
        or not _v8_firewall_stat.S_ISDIR(output_stat.st_mode)
        or (preflight_stat.st_dev, preflight_stat.st_ino)
        != (parent_preflight_stat.st_dev, parent_preflight_stat.st_ino)
        or (output_stat.st_dev, output_stat.st_ino)
        != (parent_output_stat.st_dev, parent_output_stat.st_ino)
        or preflight_stat.st_size != context.get("preflight_memfd_bytes")
        or preflight_stat.st_dev != context.get("preflight_memfd_st_dev")
        or preflight_stat.st_ino != context.get("preflight_memfd_st_ino")
        or seals != context.get("preflight_memfd_seals")
        or seals & required_seals != required_seals
        or preflight_hash != context.get("preflight_memfd_sha256")
        or preflight_payload != stage_preflight_payload
        or output_stat.st_dev != context.get("output_parent_st_dev")
        or output_stat.st_ino != context.get("output_parent_st_ino")
        or context.get("launch_manifest_sha256") != raw[13]
        or context.get("authorization_file_sha256") != raw[17]
    ):
        raise RuntimeError("V8_CORE_FIREWALL_REJECT: immutable preflight/output descriptor mismatch before NumPy import")


_v8_preimport_production_firewall(_V8_CORE_CONTEXT)
_V8_PREIMPORT_PRODUCTION_AUTHENTICATED = _V8_CORE_CONTEXT.get("mode") == "production_child"
...
# These values are part of the frozen producer contract.  A production
# authorization supplies only identities (paths and digests); it cannot choose
...
def validate_stage_manifest(stage: Path, manifest_path: Path, external_sha256: str) -> dict[str, Any]:
    ...
    if not isinstance(allowed_rows, list) or len(allowed_rows) != len(STAGE_MEMBERS_V8):
        raise RuntimeError("launch-manifest allowed-member cardinality drift")
    if len(set(allowed_rows)) != len(allowed_rows) or set(allowed_rows) != STAGE_MEMBERS_V8:
    ...
    if observed != STAGE_MEMBERS_V8:
        raise RuntimeError(f"stage membership mismatch: {sorted(observed)}")
    rows = manifest.get("members", [])
    expected_rows = STAGE_MEMBERS_V8 - {"launch_manifest.json"}
...
def write_sealed_json_descriptor(
    output_parent_descriptor: int,
    run_root_name: str,
    result_name: str,
    report: dict[str, Any],
) -> None:
    """Create and commit the result using only held directory descriptors.

    The pathname authorization gate has already closed.  Neither the output
    parent nor the newly-created run root is re-opened by an absolute path.
    """
    if (
        not run_root_name or run_root_name in {".", ".."}
        or os.sep in run_root_name or (os.altsep is not None and os.altsep in run_root_name)
        or result_name != "result.json"
    ):
        raise RuntimeError("invalid descriptor-relative output component")
    clean = dict(report)
    ...
    try:
        os.stat(run_root_name, dir_fd=output_parent_descriptor, follow_symlinks=False)
    except FileNotFoundError:
        pass
    else:
        raise RuntimeError("authorized run root was not create-new at commit")
    os.mkdir(run_root_name, mode=0o700, dir_fd=output_parent_descriptor)
    run_flags = (
        os.O_RDONLY | getattr(os, "O_CLOEXEC", 0)
        | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0)
    )
    run_descriptor = os.open(run_root_name, run_flags, dir_fd=output_parent_descriptor)
    try:
        run_metadata = os.fstat(run_descriptor)
        if not stat.S_ISDIR(run_metadata.st_mode):
            raise RuntimeError("descriptor-relative run root is not a directory")
        temporary_name = ".result.json.v8-create-new.tmp"
        flags = (
            os.O_WRONLY | os.O_CREAT | os.O_EXCL
            | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0)
        )
        descriptor = os.open(temporary_name, flags, 0o600, dir_fd=run_descriptor)
        try:
            offset = 0
            while offset < len(payload):
                written = os.write(descriptor, payload[offset:])
                if written <= 0:
                    raise RuntimeError("short descriptor-relative result write")
                offset += written
            os.fsync(descriptor)
        finally:
            os.close(descriptor)
        os.rename(
            temporary_name, result_name,
            src_dir_fd=run_descriptor, dst_dir_fd=run_descriptor,
        )
...
def build_panel(label: str, cp: Any, *, bindings: dict[str, Any] | None = None, source_dir: Path | None = None, source_moments: Sequence[dict[str, float]] | None = None, replica: int | None = None) -> dict[str, Any]:
    ...
    panel_memory_release: list[dict[str, Any]] = []
    for expert in range(EXPERTS):
        ...
        # All gathered/affine/support/XKLT consumers have completed.  Delete
        # every loop alias as well as the owning containers before proving the
        # pool has no live allocation.  In particular, `masks`, `x`, and
        # `words` otherwise survive as Python loop locals.
        del masks, pair_masks, pair_x, pair_words
        if bindings is None:
            del x, words
        del pair_tables, control_meta, pair_components, candidates, moment
        ...
        if used_before_free != 0:
            raise RuntimeError(
                f"panel build leaked live pool bytes before free at expert={expert}: "
                f"{used_before_free}"
            )
        ...
        if used_after_free != 0 or total_after_free != 0:
            raise RuntimeError(
                f"panel build pool did not close at expert={expert}: "
                f"used={used_after_free} total={total_after_free}"
            )
        panel_memory_release.append({
            ...
            "all_per_expert_gpu_arrays_deleted_before_free": True,
        })
        del pool
    ...
    return {
        ...
        "memory_release": panel_memory_release,
...
def search_panel(panel: dict[str, Any], maximum_passes: int = 4) -> dict[str, Any]:
    if not isinstance(maximum_passes, int) or isinstance(maximum_passes, bool) or maximum_passes != 4:
        raise RuntimeError("v8 search requires exactly four maximum coordinate passes")
    output: dict[str, Any] = {}
    for rate in RATES:
...
def validate_external_audit(section: dict[str, Any], label: str) -> tuple[Path, Path]:
    require_exact_keys(section, {
        ...
        "receipt_internal_field", "receipt_internal_sha256",
    }, label)
    frozen = {
        "source audit": (
            SOURCE_AUDIT_MANIFEST_SCHEMA, SOURCE_AUDIT_MANIFEST_STATUS,
            SOURCE_AUDIT_RECEIPT_SCHEMA, SOURCE_AUDIT_PASS_STATUS,
        ),
        "runtime audit": (
            RUNTIME_AUDIT_MANIFEST_SCHEMA, RUNTIME_AUDIT_MANIFEST_STATUS,
            RUNTIME_AUDIT_RECEIPT_SCHEMA, RUNTIME_AUDIT_PASS_STATUS,
        ),
    }
    if label not in frozen:
        raise RuntimeError("external audit label is not frozen")
    manifest_schema, manifest_status, receipt_schema, receipt_status = frozen[label]
    ...
    manifest_value = strict_json_bytes(manifest_payload, f"{label} manifest")
    if manifest_value.get("schema") != manifest_schema or manifest_value.get("status") != manifest_status:
        raise RuntimeError(f"{label} manifest schema/status is not the frozen PASS contract")
    receipt, _, value = read_external_json(section["receipt_path"], section["receipt_file_sha256"], f"{label} receipt")
    if value.get("schema") != receipt_schema or value.get("status") != receipt_status:
        raise RuntimeError(f"{label} receipt schema/status is not the frozen PASS contract")
...
def validate_runtime_receipt(section: dict[str, Any], runtime_contract_sha256: str) -> tuple[Path, dict[str, Any]]:
    require_exact_keys(section, {
        "path", "file_sha256", "internal_sha256", "runtime_contract_file_sha256",
    ...
    if receipt.get("schema") != "lossy-tail-v8-source-free-runtime-receipt-v1":
        raise RuntimeError("runtime receipt schema mismatch")
    if receipt.get("status") != RUNTIME_RECEIPT_STATUS:
...
def validate_production_authorization(
    ...
    *, stage: Path,
    ...
    if output_parent_descriptor < 3:
        raise RuntimeError("held output-parent descriptor is invalid")
    output_parent_metadata = os.fstat(output_parent_descriptor)
    if (
        not stat.S_ISDIR(output_parent_metadata.st_mode)
        or (output_parent_metadata.st_dev, output_parent_metadata.st_ino) != output_parent_identity
    ):
        raise RuntimeError("held output-parent descriptor/context identity mismatch")
    ...
    if authorization["schema"] != "lossy-tail-v8-one-shot-production-authorization-v1":
    ...
    if stage_row != {
        ...
        "launch_manifest_internal_stage_member_count": len(STAGE_MEMBERS_V8),
    }:
    ...
    run_root_raw = output_row["run_root"]
    result_raw = output_row["result_path"]
    if (
        not isinstance(run_root_raw, str) or not isinstance(result_raw, str)
        or not run_root_raw or not result_raw
        or "\x00" in run_root_raw or "\x00" in result_raw
        or not os.path.isabs(run_root_raw) or not os.path.isabs(result_raw)
        or run_root_raw != os.path.normpath(run_root_raw)
        or result_raw != os.path.normpath(result_raw)
    ):
        raise RuntimeError("authorized output must use absolute lexical-canonical spelling")
    ...
    if (
        not run_root_name or run_root_name in {".", ".."}
        or os.sep in run_root_name or (os.altsep is not None and os.altsep in run_root_name)
        or result != run_root / "result.json"
    ):
    ...
    for row in identity_rows:
        ...
        metadata = (
            ...
            if row["label"] == "output_existing_parent"
            else os.stat(live, follow_symlinks=False)
        )
    ...
    try:
        ...
        raise RuntimeError("authorized run root/result are not create-new")
    return {
...
def self_test() -> None:
    def require(condition: bool, label: str) -> None:
        if not condition:
            raise RuntimeError(f"v8 self-test failed: {label}")
...
def main() -> None:
    if not sys.flags.isolated or not sys.dont_write_bytecode or sys.flags.optimize != 0:
        raise RuntimeError("v8 production core requires python -B -I without optimization")
    context = _V8_CORE_CONTEXT
    if not _V8_PREIMPORT_PRODUCTION_AUTHENTICATED:
        raise RuntimeError("v8 production main was not authenticated before NumPy import")
    require_exact_keys(context, {
        ...
        "launch_manifest_sha256",
        "authorization_file_sha256", "authorization_internal_sha256",
    }, "v8 production child context")
    if context["mode"] != "production_child":
        raise RuntimeError("v8 production main requires a production-child context")
    if context["child_pid"] != os.getpid() or context["parent_pid"] != os.getppid():
        raise RuntimeError("v8 production child process identity mismatch")
    _v8_preimport_production_firewall(context)
    for label in (
        "capability_sha256", "preflight_memfd_sha256", "launch_manifest_sha256",
        "authorization_file_sha256", "authorization_internal_sha256",
    ):
        value = context[label]
        if not isinstance(value, str) or len(value) != 64 or any(ch not in "0123456789abcdef" for ch in value):
            raise RuntimeError(f"invalid v8 production child context hash: {label}")
    production_flags = [
        "--bindings", "--protocol", "--repair-lock", "--runtime-contract",
        "--authorization-contract", "--launch-manifest", "--launch-manifest-sha256",
        "--authorization", "--authorization-sha256", "--control-replicates",
    ...
    if len(raw) != 2 * len(production_flags) or raw[::2] != production_flags:
        raise RuntimeError("invalid frozen v8 production-core grammar")
    if raw[19] != "4" or raw[21] != "4":
        raise RuntimeError("frozen v8 production requires controls=4 and passes=4")
    ...
    if protocol.get("status") != "FROZEN_V8_BEFORE_ANY_RUNTIME_CALIBRATION_PAYLOAD_OR_GPU_EXECUTION":
    ...
    if repair.get("schema") != "lossy-tail-release-repair-lock-v8" or repair.get("status") != "FROZEN_V8_SOURCE_PACKAGE_NO_RUNTIME_OR_PRODUCTION_AUTHORIZATION":
        raise RuntimeError("v8 repair lock is not frozen")
    ...
    authorization_state = validate_production_authorization(
        ...
        output_parent_descriptor=int(context["output_parent_fd"]),
        output_parent_identity=(int(context["output_parent_st_dev"]), int(context["output_parent_st_ino"])),
    )
    ...
    for replica in range(args.control_replicates):
        ...
        control_meta.append({
            ...
            "memory_release": panel["memory_release"],
        })
        del panel
    ...
    report = {
        "schema": "qwen_lossy_tail_peeling_oracle_result_v8",
        "authorization": {
        ...
        "child_capability": {
            ...
            "preflight_memfd_sha256": context["preflight_memfd_sha256"],
            "preflight_memfd_st_dev": context["preflight_memfd_st_dev"],
            "preflight_memfd_st_ino": context["preflight_memfd_st_ino"],
            "preflight_memfd_seals": context["preflight_memfd_seals"],
            "output_parent_st_dev": context["output_parent_st_dev"],
            "output_parent_st_ino": context["output_parent_st_ino"],
        },
        ...
        "panel": {
            ...
            "memory_release": qwen_panel["memory_release"],
        },
    ...
    write_sealed_json_descriptor(
        authorization_state["output_parent_descriptor"],
        authorization_state["run_root_name"],
        authorization_state["result_name"],
        report,
    )
