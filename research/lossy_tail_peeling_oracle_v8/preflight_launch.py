#!/usr/bin/env python3
"""V8 stdlib release preflight and one-use child-capability issuer."""
...
except ImportError:  # Windows source-audit/test import only; production is Linux-only.
...
def fail(message: str) -> "NoReturn":  # type: ignore[name-defined]
    raise SystemExit(f"V8_PREFLIGHT_REJECT: {message}")
...
def validate_sealed_preflight_descriptor(descriptor: int, expected: bytes) -> dict[str, Any]:
    if fcntl is None or descriptor < 3:
        fail("sealed preflight descriptor is unavailable")
    metadata = os.fstat(descriptor)
    if not stat.S_ISREG(metadata.st_mode) or metadata.st_size != len(expected):
        fail("sealed preflight descriptor type/size mismatch")
    seals = int(fcntl.fcntl(descriptor, fcntl.F_GET_SEALS))
    required = required_memfd_seals()
    if seals & required != required:
        fail("sealed preflight descriptor lacks immutable seals")
    payload = descriptor_payload(descriptor, metadata.st_size, "sealed preflight")
    if payload != expected:
        fail("sealed preflight bytes differ from authenticated stage bytes")
    return {
        "fd": descriptor,
...
def create_sealed_preflight(payload: bytes) -> int:
    if fcntl is None or not hasattr(os, "memfd_create"):
        fail("Linux memfd_create is required")
    flags = int(getattr(os, "MFD_ALLOW_SEALING", 0x0002)) | int(getattr(os, "MFD_CLOEXEC", 0x0001))
    descriptor = os.memfd_create("lossy_tail_v8_authenticated_preflight", flags)
    try:
        offset = 0
        ...
        validate_sealed_preflight_descriptor(descriptor, payload)
...
def validate_audit(
    ...
    *,
    ...
    if value.get("schema") != receipt_schema or value.get("status") != receipt_status:
        fail(f"{label} receipt schema/status is not the frozen PASS contract")
...
def main() -> None:
    ...
    if len(raw) == 8 and raw[::2] == flags:
        ...
        _, initial_payloads = validate_stage(stage, manifest_path, manifest_sha256)
        sealed_descriptor = create_sealed_preflight(initial_payloads["preflight_launch.py"])
        descriptor_entry = f"/proc/self/fd/{sealed_descriptor}"
        argv = [
            sys.executable, "-B", "-I", descriptor_entry,
            "--sealed-preflight-fd", str(sealed_descriptor), *raw,
        ]
        os.execve(sys.executable, argv, dict(os.environ))
        fail("descriptor re-exec unexpectedly returned")

    if len(raw) != 10 or raw[:1] != ["--sealed-preflight-fd"] or raw[2::2] != flags:
        fail("exact external grammar plus one internal sealed-preflight descriptor is required")
    try:
        sealed_descriptor = int(raw[1], 10)
    except ValueError:
        fail("sealed preflight fd must be a canonical decimal integer")
    if sealed_descriptor < 3 or str(sealed_descriptor) != raw[1]:
        fail("sealed preflight fd must be canonical and >=3")
    descriptor_entry = f"/proc/self/fd/{sealed_descriptor}"
    if sys.argv[0] != descriptor_entry or os.fspath(Path(__file__)) != descriptor_entry:
        fail("authenticated preflight must execute directly from its sealed descriptor")
    raw = raw[2:]
    manifest_path = canonical_original(raw[1], "manifest", allow_missing_tail=False)
    stage = canonical_original(os.fspath(manifest_path.parent), "stage", allow_missing_tail=False)
    ...
    sealed_identity = validate_sealed_preflight_descriptor(sealed_descriptor, payloads["preflight_launch.py"])
    ...
    if authorization.get("schema") != "lossy-tail-v8-one-shot-production-authorization-v1" or authorization.get("status") != "AUTHORIZED_ONCE_AFTER_INDEPENDENT_SOURCE_AND_RUNTIME_AUDITS":
    ...
    source_audit_manifest, source_audit_receipt = validate_audit(
        authorization["source_audit"], "source audit",
        manifest_schema=SOURCE_AUDIT_MANIFEST_SCHEMA,
        manifest_status=SOURCE_AUDIT_MANIFEST_STATUS,
        receipt_schema=SOURCE_AUDIT_RECEIPT_SCHEMA,
        receipt_status=SOURCE_AUDIT_PASS_STATUS,
    )
    ...
    exact_keys(runtime_section, {"path", "file_sha256", "internal_sha256", "runtime_contract_file_sha256"}, "runtime receipt")
    ...
    if runtime_receipt.get("schema") != "lossy-tail-v8-source-free-runtime-receipt-v1" or runtime_receipt.get("status") != RUNTIME_RECEIPT_STATUS:
    ...
    runtime_audit_manifest, runtime_audit_receipt = validate_audit(
        authorization["runtime_audit"], "runtime audit",
        manifest_schema=RUNTIME_AUDIT_MANIFEST_SCHEMA,
        manifest_status=RUNTIME_AUDIT_MANIFEST_STATUS,
        receipt_schema=RUNTIME_AUDIT_RECEIPT_SCHEMA,
        receipt_status=RUNTIME_AUDIT_PASS_STATUS,
    )
    ...
    output_parent_descriptor = os.open(os.fspath(run_root.parent), output_parent_flags)
    output_parent_stat = os.fstat(output_parent_descriptor)
    output_identity = next(row for row in rows if row["label"] == "output_existing_parent")
    if (
        not stat.S_ISDIR(output_parent_stat.st_mode)
        or (output_parent_stat.st_dev, output_parent_stat.st_ino)
        != (output_identity["st_dev"], output_identity["st_ino"])
    ):
        os.close(output_parent_descriptor)
        fail("held output-parent descriptor identity mismatch")
    print(json.dumps({
        "v8_production_preflight": "PASS_AUTHORIZATION_BOUND_BEFORE_CHILD_CAPABILITY",
    ...
    child_flags = [
        ...
        "--preflight-memfd-fd", str(sealed_descriptor),
        "--output-parent-fd", str(output_parent_descriptor),
    ]
    oracle_path = stage / "lossy_tail_oracle.py"
    supplemental_parent_executable = os.stat("/proc/self/exe")
    ...
    try:
        ...
        process = subprocess.Popen(
            command,
            close_fds=True,
            pass_fds=(child_descriptor, sealed_descriptor, output_parent_descriptor),
            env=dict(os.environ),
        )
        child_channel.close()
        capability = {
            "schema": "lossy-tail-v8-one-use-child-capability-v1",
            "status": "ISSUED_ONCE_BY_AUTHENTICATED_PREFLIGHT",
            "parent_pid": os.getpid(),
            "child_pid": process.pid,
            "nonce_hex": secrets.token_hex(32),
            "preflight_memfd_fd": sealed_identity["fd"],
            "preflight_memfd_st_dev": sealed_identity["st_dev"],
            "preflight_memfd_st_ino": sealed_identity["st_ino"],
            "preflight_memfd_bytes": sealed_identity["bytes"],
            "preflight_memfd_seals": sealed_identity["seals"],
            "preflight_memfd_sha256": sealed_identity["sha256"],
            "output_parent_fd": output_parent_descriptor,
            "output_parent_st_dev": output_parent_stat.st_dev,
            "output_parent_st_ino": output_parent_stat.st_ino,
            "supplemental_parent_executable_st_dev": supplemental_parent_executable.st_dev,
            "supplemental_parent_executable_st_ino": supplemental_parent_executable.st_ino,
    ...
    print(json.dumps({
        "v8_production_child": "EXITED_SUCCESS_AFTER_ONE_USE_CAPABILITY",
        "child_pid": process.pid,
        "capability_sha256": capability_sha256,
