# QSB-PTQ-v1 source-only stage-0 design
...
`QSB-PTQ-v1` divides every 768-by-2048 scored matrix into 64-value blocks. A
...
QSB-PTQ-v1 preserves every v0 panel binding, topology, physical cell, seed,
oracle, and control. It changes only the ideal-KL budget policy and truthful
status/margin labels. Alpha is fitted to 96.5% of the physical expert payload;
the separate execution limit is 97%, leaving a preregistered 0.5 percentage
point calibration/generalization cushion. The true physical reservoir remains
100%. A 97%-limit rejection must not be called physical overflow unless ideal
KL independently exceeds 100%.

This budget split was chosen after inspecting the v0 result on this same Qwen
panel. It is frozen before any v1 Qwen/GPU execution, but a future v1 run remains
an adapted source-leaking engineering diagnostic—not independent held-out or
confirmatory evidence.

Strict ordering is frozen:

1. Fit alpha to 96.5% on fit experts. Before decoder fitting, use a physical
   overflow status only above 100%; otherwise kill above the separately frozen
   97% execution limit with a policy-limit status. Report margins to both.
