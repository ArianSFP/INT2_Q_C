#!/usr/bin/env python3
"""Standard-library contracts and exact CPU reference for UWFA census v6.
...
DESIGN_SCHEMA = "unifilar-wfa-entropy-census-design-v6"
RESULT_SCHEMA = "unifilar-wfa-entropy-census-result-v6"
REVIEW_SCHEMA = "unifilar-wfa-entropy-census-independent-source-review-v6"
STREAM_LOCK_SCHEMA = "unifilar-wfa-canonical-stream-lock-v6"
CONTROL_LOCK_SCHEMA = "unifilar-wfa-gaussian-control-panel-lock-v6"
AUTHORIZATION = "OPEN_AUTHENTICATED_UNIFILAR_WFA_CENSUS_AFTER_INDEPENDENT_SOURCE_REVIEW_V6"
...
SPLIT_HASH_SEED = b"UWFA-V6-NESTED-SPLIT-2026-09-02"
...
def deserialize_model(packet: bytes) -> tuple[Candidate, list[int]]:
    ...
    require(magic == b"UWFAV6\x00\x00" and version == 3, "model magic/version")
...
def parent_commit_marker_name(final_name: str) -> str:
    ...
    digest = sha256_bytes(b"UWFA-V6-COMMIT-NAME\x00" + final_name.encode("utf-8"))
...
def committed_directory_root(source_manifest_sha256: str, members: list[dict[str, Any]]) -> str:
    ...
    return sha256_bytes(b"UWFA-V6-HELD-DIRECTORY-ROOT\x00" + body)
...
def _link_held_unnamed_file_noreplace(source_fd: int, parent_fd: int, destination: str) -> None:
    ...
    result = linkat(source_fd, b"", parent_fd, os.fsencode(destination), at_empty_path)
    if result != 0:
        error = ctypes.get_errno()
        ...
        if error in {errno.ENOENT, errno.EPERM, errno.EACCES}:
            ...
            result = linkat(at_fdcwd, proc_fd, parent_fd, os.fsencode(destination), at_symlink_follow)
        if result != 0:
            error = ctypes.get_errno()
            raise OSError(error, os.strerror(error), destination)
...
class CompletionLastOutput:
    ...
    def __init__(self, parent: RetainedOutputParent, final_name: str, transaction_id: str):
        ...
        anchor_suffix = sha256_bytes(b"UWFA-V6-COMMIT-ANCHOR\x00" + final_name.encode("utf-8"))[:16]
        self.commit_anchor_name = f".uwfa-commit-anchor-v6-{self.transaction_id}-{anchor_suffix}.incomplete"
    ...
    def __enter__(self) -> "CompletionLastOutput":
        ...
        self.write_new("RUN_STATE.json", pretty_json({
            "schema": "unifilar-wfa-run-state-v6",
            "complete": False,
    ...
    def _parent_commit_record(
        ...
        *,
        ...
        return seal_record(
            {
                "schema": "unifilar-wfa-parent-commit-v6",
    ...
    def complete(self, members: list[dict[str, Any]], source_manifest_sha256: str) -> dict[str, Any]:
        ...
        try:
            ...
            while len(observed_marker) < len(marker_bytes):
                chunk = os.read(marker_fd, min(1 << 20, len(marker_bytes) - len(observed_marker)))
                require(chunk, "short held commit-marker read")
                observed_marker.extend(chunk)
            require(bytes(observed_marker) == marker_bytes and os.read(marker_fd, 1) == b"", "held commit-marker byte mismatch")

            # Repeat both bindings immediately before the fd-addressed link.
            self._verify_final_name_binding()
            self._verify_exact_staging_members(list(self.members))
            self.parent.verify_stable()
            _link_held_unnamed_file_noreplace(marker_fd, self.parent.fd, self.commit_marker_name)
