#!/usr/bin/env python3
"""Native verifier for a post-review sealed UWFA-SC v6 source package."""
...
POST_FREEZE_REQUIREMENTS = [
    "EXTERNAL_PINNED_V6_SOURCE_AUDIT",
...
def verify_package(package: Path) -> dict[str, Any]:
    ...
    require(manifest["schema"] == "unifilar-wfa-source-manifest-v6", "manifest schema")
    ...
    require(design.get("schema") == "unifilar-wfa-entropy-census-design-v6", "design schema")
    ...
    common = module_from_snapshot("uwfa_verify_common_v6", snapshots["uwfa_common.py"])
    ...
    require(model.startswith(b"UWFAV6\x00\x00"), "v6 serialized model")
    ...
    for token in (
        ...
        require(token in stage_text, f"v6 source gate: {token}")
    ...
    for token in (
        ...
        "/proc/self/fd/", "def committed_directory_root",
        "UWFA-V6-HELD-DIRECTORY-ROOT", "unifilar-wfa-parent-commit-v6",
    ):
        require(token in common_text, f"descriptor-relative publication contract: {token}")
    require(common_text.count("self._verify_exact_staging_members(") == 5, "held-directory verification at every V6 publication boundary")
    ...
    for token in (
        ...
        require(token in tests_text, f"V6 hostile regression: {token}")

    return {
        "schema": "unifilar-wfa-source-verification-v6",
