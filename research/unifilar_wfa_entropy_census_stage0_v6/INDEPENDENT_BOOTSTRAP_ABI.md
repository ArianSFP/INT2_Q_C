# UWFA-SC v6 independent bootstrap ABI
...
- the SHA-256 of an independent v6 source-review receipt whose schema is
  `unifilar-wfa-entropy-census-independent-source-review-v6`;
...
- `container_codec.py`: literal `UWFCV6` build/parse/rebuild/routed-read ledger;
...
9. Construct a canonical UTF-8-sorted member table including `COMPLETE.json`
   ...
   and marker fds, rehash the directory, and fsync the parent. Do not delete or
...
10. Independent consumers must receive a retained parent descriptor, derive and
   ...
   Recheck both names against held fds after verification; consume only held or
   already-buffered authenticated bytes.
...
The literal result also carries a `uwfa-sc-v6-posterior-diagnostic-handoff`
