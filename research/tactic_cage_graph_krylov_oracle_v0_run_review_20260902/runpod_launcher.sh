...
CUDA_VISIBLE_DEVICES=0 /workspace/int2-cupy-venv/bin/python -I -B \
  /tmp/tactic_cage_graph_krylov_oracle_v0_4d5220fe/run_oracle.py \
  ...
  --output-dir /workspace/tactic_cage_graph_krylov_oracle_v0_qwen_result_20260902_4d5220fe \
  > /tmp/tactic_cage_graph_krylov_oracle_v0_qwen_4d5220fe.log 2>&1
RUN_STATUS=$?
printf '%s\n' "$RUN_STATUS" > /tmp/tactic_cage_graph_krylov_oracle_v0_qwen_4d5220fe.exit
exit "$RUN_STATUS"
