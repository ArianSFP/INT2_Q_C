...
SCHEMA = "silwarp_auxiliary_protocol_v2"
SOURCE_LOCK_SCHEMA = "silwarp_auxiliary_source_lock_v1"
SENTINEL_SCHEMA = "silwarp_launch_sentinel_v2"
AUTHORIZATION_PHRASE = "AUTHORIZE_SILWARP_AUXILIARY_IDEAL_CHANNEL_RUN_V2"
MODEL_MAGIC = "SILWARP_MODEL_V2"
EXPERT_MAGIC = b"SILWEX2\0"
...
def load_protocol(path: Path = PROTOCOL_PATH) -> dict[str, Any]:
    ...
    if protocol.get("status") != "FROZEN_BEFORE_V2_PAYLOAD_OR_CUDA":
...
def canonical_fp16_mean(exact_mean: float) -> np.float16:
    """Nearest-even FP16 mean with a unique +0 representation.

    IEEE +0 and -0 are numerically identical.  Canonicalizing zero therefore
    changes no normalization value or information while making charged
    metadata and matched-control bit comparisons deterministic.
    """

    exact_mean = require_finite_scalar("exact source mean", exact_mean)
    with np.errstate(over="ignore", invalid="ignore"):
        mean16 = np.float16(exact_mean)
    if not np.isfinite(mean16):
        raise OverflowError("FP16 mean overflow")
    if mean16 == np.float16(0.0):
        mean16 = np.float16(0.0)
    return mean16
...
def upward_fp16_moments(values: Any) -> dict[str, Any]:
    ...
    mean16 = canonical_fp16_mean(exact_mean)
