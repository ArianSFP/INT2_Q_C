"""No-payload, no-CUDA tests for the frozen SILWARP-v2 gate."""
...
class InformationAndMomentTests(unittest.TestCase):
    ...
    def test_signed_zero_mean_is_canonicalized_without_numeric_change(self) -> None:
        # This is the exact mean observed for the already-authenticated v1
        # failure record L31/E31/down.  Nearest-even FP16 produces -0, while
        # v2 gives zero a unique charged representation before RMS evaluation.
        real_offending_mean = -4.649028828573876e-09
        raw_mean16 = np.float16(real_offending_mean)
        self.assertEqual(int(raw_mean16.view(np.uint16)), 0x8000)
        canonical = sw.canonical_fp16_mean(real_offending_mean)
        self.assertEqual(int(canonical.view(np.uint16)), 0x0000)
        self.assertEqual(float(raw_mean16), float(canonical))

        adversarial = np.asarray([-1.0e-8, 0.0], dtype=np.float64)
        moments = sw.upward_fp16_moments(adversarial)
        self.assertEqual(
            int(moments["serialized_mean_fp16"].view(np.uint16)), 0x0000
        )
        normalized = sw.normalize_with_serialized_moments(adversarial, moments)
        self.assertLessEqual(
            float(np.mean(normalized.astype(np.float64) ** 2)), 1.0
        )

    def test_real_v1_failure_moment_accepts_both_unchanged_controls(self) -> None:
        # Source-free replay of the exact offending serialized moment.  This
        # invokes the production control generator and both fixed domains; it
        # does not read a payload or alter any seed.
        moments = {
            "serialized_mean_fp16": sw.canonical_fp16_mean(
                -4.649028828573876e-09
            ),
            "serialized_rms_fp16": np.float16(0.0242156982421875),
        }
        for control_name in sw.CONTROL_NAMES:
            tiles, energy = gate.moment_matched_control(
                (31, 31, "down"), control_name, moments
            )
            self.assertTrue(math.isfinite(energy))
            self.assertLessEqual(
                float(np.mean(tiles.astype(np.float64) ** 2)), 1.0
            )

    def test_bitwise_assertion_still_rejects_noncanonical_or_nonzero_mismatch(self) -> None:
        gate.require_identical_fp16_bits("mean", np.float16(0.0), np.float16(0.0))
        with self.assertRaisesRegex(AssertionError, "serialized mean mismatch"):
            gate.require_identical_fp16_bits(
                "mean", np.float16(-0.0), np.float16(0.0)
            )
        one = np.float16(1.0)
        next_one = np.nextafter(one, np.float16(np.inf), dtype=np.float16)
        with self.assertRaisesRegex(AssertionError, "serialized RMS mismatch"):
            gate.require_identical_fp16_bits("RMS", one, next_one)
...
class ModelTests(unittest.TestCase):
    ...
    def test_two_independent_nulls_and_fixed_training_seeds(self) -> None:
        ...
        # These v1 domain values are locked so the representation-only repair
        # cannot silently change either predeclared control realization.
        self.assertEqual(
            sw.derive_seed("control-source", "null_a", 31, 31, "down"),
            4217512476569775294,
        )
        self.assertEqual(
            sw.derive_seed("control-source", "null_b", 31, 31, "down"),
            10624511461204386588,
        )
