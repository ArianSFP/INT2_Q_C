...
from search import CompleteRateCap, search_bank
...
def main() -> None:
    ...
    geometry = Geometry(rows=704, cols=2304, role=2, row_start=512,
                        row_count=16, col_start=1792, col_count=256)
    source, levels, distortion = synthetic_source(geometry)
    lambda_bit = 2.5e-5
    rate_cap = CompleteRateCap(total_weights=geometry.count, outer_bits=0)
    structured = search_bank(distortion, geometry, lambda_bit, rate_cap)
    ...
    control = search_bank(control_distortion, geometry, lambda_bit, rate_cap)
    role_receipts = {}
    for role, row_start in enumerate((0, 256, 512)):
        role_geometry = Geometry(704, 2304, role, row_start, 16, 1792, 256)
    ...
    result = {
        "schema": "strata-bmp-obdd-qtt6-source-free-fixture-v1-hardened",
        "status": "EXECUTION_PENDING__SOURCE_ONLY__HOLD_PAYLOAD",
        ...
        "complete_rate_cap": structured["complete_rate_cap"],
