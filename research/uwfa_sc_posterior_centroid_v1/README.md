...
The historical v0 launch failed closed before producing a result. The exact
result-bound STRATA function returns each group ordinal as `numpy.int64`, while
the unchanged authenticated v8 adapter requires `type(group) is int`. The
failure was therefore an ABI-type mismatch at the inherited-metadata unpack
boundary, not a Qwen posterior-centroid measurement.

Version 1 does not edit v0, v8, the v9 publication, or the failed v0 namespace.
`PythonIntOrdinalStrataBridge` wraps the authenticated STRATA module and changes
only the return representation of `expected_block_group_ordinals`:

```text
NumPy integral scalar --operator.index--> concrete Python int
```

Before returning immutable tuples, it verifies block count, the group count
implied by every `BLOCK_LOG2`, ordinal range, and exact unique coverage of all
groups. Floats, strings, booleans, out-of-range values, duplicates and missing
groups are rejected. Labels and group ordering are unchanged. The source-only
regression first proves that the exact v8 `unpack_immutable_metadata` route
fails with `np.int64`, then proves that the same retained v8 bytes pass through
this narrow bridge.
...
no 64-entry source-fitted centroid table in v1. The local control retains only
...
The maximum v1 head is 1,636 bytes including its 96-byte header, so every head
...
There is deliberately **no inference-ready routed posterior decoder in v1**.
...
```bash
python -I -B \
  research/uwfa_sc_posterior_centroid_v1/test_source_only.py

python -I -B \
  research/uwfa_sc_posterior_centroid_v1/verify_source.py \
  --package research/uwfa_sc_posterior_centroid_v1 \
...
SHA-256 explicitly. A launch is still blocked until the corrected independent
v9 result audit has emitted a passing receipt and a separate reviewer has
sealed one launch-review JSON record. The output directory must not already
exist. The runner authenticates its own complete source closure, then the audit
receipt, then the launch review, all before even statting the completed Qwen v9
publication. It binds every publication member to the passing receipt before
importing NumPy/CuPy, and authenticates the publication before opening any BF16
score source. `posterior_core.py` and `result_bridge.py` are compiled from the
retained bytes authenticated by that closure; they are not imported later from
mutable sibling paths.

```bash
python -I -B research/uwfa_sc_posterior_centroid_v1/diagnostic.py \
  --authorization RUN_UWFA_SC_POSTERIOR_CENTROID_V1_DISCOVERY \
  --package-manifest-sha256 <audited-package-manifest-sha256> \
  --predecessor-audit-receipt <passing-independent-result-audit.json> \
  --predecessor-audit-receipt-sha256 <passing-audit-receipt-sha256> \
  --launch-review <one-run-launch-review.json> \
  --launch-review-sha256 <launch-review-sha256> \
...
The launch review has exact schema
`uwfa-sc-posterior-centroid-v1-launch-review-v0` and status
`APPROVED_FOR_ONE_NONPROMOTING_DISCOVERY_RUN`. It binds the audit receipt,
posterior source manifest, score manifest, literal path strings, output
namespace, `rht_device`, and `output_directory_must_not_exist=true`. It grants
no positive-claim authority.

## Control and claim boundary

The state-aware, local-only and state-permuted laws retain identical nested
whole-owner cross-fitting, binary16 round-trip and physical-byte charging. The
diagnostic still reports exact original-source posterior-centroid MSE. It does
not run the eight matched-Gaussian pipelines itself: those must repeat the
complete fit, selection and serialization procedure, remain separately
authenticated, and pass before promotion. A v1 Qwen run is discovery evidence
only and cannot establish universal SwiGLU-MoE performance.
