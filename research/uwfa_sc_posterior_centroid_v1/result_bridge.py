...
Version 1 adds one deliberately narrow ABI bridge around the authenticated
STRATA module.  The historical implementation returns NumPy integer scalars
from ``expected_block_group_ordinals`` while the unchanged v8 adapter quite
properly accepts only concrete Python ``int`` objects at its strict boundary.
The bridge converts values through the integral ``__index__`` protocol,
validates the complete block geometry and exact group coverage, and returns
immutable tuples of built-in integers.  It does not change labels, ordering,
decoder arithmetic, weights, or any authenticated predecessor source bytes.
"""
...
ORDINAL_BRIDGE_SCHEMA = "uwfa-sc-python-int-group-ordinal-bridge-v1"


class PythonIntOrdinalStrataBridge:
    """Read-only module proxy normalizing group ordinals at one ABI boundary.

    ``operator.index`` is intentionally used instead of ``int(value)`` so
    floats, numeric strings, and other lossy coercions remain fail-closed.
    The output is additionally checked against the same complete coverage
    required later by the v8 reconstruction path.  Attribute lookup otherwise
    delegates to the exact manifest-authenticated STRATA module.
    """

    __slots__ = ("_base",)

    def __init__(self, authenticated_base: Any) -> None:
        require(
            getattr(authenticated_base, "__authenticated_sha256__", None) is not None,
            "ordinal bridge authenticated STRATA base",
        )
        self._base = authenticated_base

    def __getattr__(self, name: str) -> Any:
        require(name not in {"_base", "__dict__"}, "ordinal bridge private attribute")
        return getattr(self._base, name)

    @staticmethod
    def _python_int(value: Any) -> int:
        require(type(value) is not bool, "STRATA group ordinal boolean")
        require(type(value).__name__ != "bool_", "STRATA group ordinal boolean scalar")
        try:
            indexed = operator.index(value)
        except TypeError as error:
            raise BridgeError("STRATA group ordinal integral scalar") from error
        result = int(indexed)
        require(type(result) is int, "STRATA normalized group ordinal")
        return result

    def expected_block_group_ordinals(self, labels: Sequence[int]) -> tuple[tuple[int, ...], ...]:
        raw = self._base.expected_block_group_ordinals(labels)
        try:
            rows = tuple(raw)
        except TypeError as error:
            raise BridgeError("STRATA group map outer sequence") from error
        blocks = int(self._base.BLOCKS)
        groups = int(self._base.GROUPS)
        group_values = int(self._base.GROUP_VALUES)
        block_log2 = tuple(int(value) for value in self._base.BLOCK_LOG2)
        require(len(rows) == blocks == len(block_log2), "STRATA group map block count")
        require(groups > 0 and group_values > 0, "STRATA group geometry positive")
        ...
        for ordinal, row in enumerate(rows):
            try:
                ...
                raise BridgeError("STRATA group map row sequence") from error
            weights = 1 << block_log2[ordinal]
            require(weights % group_values == 0, "STRATA block/group divisibility")
            require(
                len(values) == weights // group_values,
                "STRATA group map row length",
            )
            converted = tuple(self._python_int(value) for value in values)
            require(
                all(0 <= value < groups for value in converted),
                "STRATA group ordinal range",
            )
            normalized.append(converted)
            covered.extend(converted)
        require(len(covered) == groups, "STRATA group map total coverage")
        require(
            sorted(covered) == list(range(groups)),
            "STRATA group map exact unique coverage",
        )
        return tuple(normalized)

    def receipt(self) -> dict[str, Any]:
        return {
            "schema": ORDINAL_BRIDGE_SCHEMA,
            "authenticated_strata_sha256": require_digest(
                getattr(self._base, "__authenticated_sha256__", None),
                "ordinal bridge STRATA base",
            ),
            "input_integral_protocol": "operator.index",
            "output_concrete_type": "builtins.int",
            "block_geometry_checked": True,
            "exact_group_coverage_checked": True,
            "predecessor_source_mutated": False,
        }
...
def load_authenticated_decoders(
    ...
    *,
    ...
    common = _load_module("uwfa_pc_v1_common", sources["uwfa_common.py"], hashes["uwfa_common.py"])
    codec = _load_module("uwfa_pc_v1_codec", sources["container_codec.py"], hashes["container_codec.py"])
    semantic = _load_module("uwfa_pc_v1_semantic", sources["universal_adapter.py"], hashes["universal_adapter.py"])
    adapter_source = _load_module("uwfa_pc_v1_adapter", sources["strata_sc_adapter.py"], hashes["strata_sc_adapter.py"])
    ...
    authenticated_strata = _load_module("uwfa_pc_v1_strata_common", strata_payload, expected_strata)
    strata = PythonIntOrdinalStrataBridge(authenticated_strata)
    frozen = _load_module("uwfa_pc_v1_frozen_auditor", frozen_payload, expected_frozen)
    return {
        "common": common,
        "codec": codec,
        "semantic": semantic,
        ...
        "strata": strata,
        "authenticated_strata_base": authenticated_strata,
        "ordinal_bridge": strata.receipt(),
        "frozen": frozen,
        "source_hashes": {
