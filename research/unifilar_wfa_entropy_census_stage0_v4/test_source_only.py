#!/usr/bin/env python3
"""Hostile source-only UWFA-SC v4 producer tests.
...
common = load("uwfa_v4_test_common", "uwfa_common.py")
protocol = load("uwfa_v4_test_protocol", "protocol.py")
semantic = load("uwfa_v4_test_semantic", "universal_adapter.py")
codec = load("uwfa_v4_test_container", "container_codec.py")
fixture = load("uwfa_v4_test_fixture", "fixture_portability.py")
strata = load("uwfa_v4_test_strata", "strata_sc_adapter.py")
stage = load("uwfa_v4_test_stage", "stage0_census.py")
cupy_backend = load("uwfa_v4_test_cupy", "cupy_backend.py")
dispatcher = load("uwfa_v4_test_dispatcher", "dispatcher_contract.py")
envelope = load("uwfa_v4_test_envelope", "result_envelope.py")
...
def valid_source_preflight(*, full: str = "aa" * 32, structural: str = "bb" * 32, score_bytes: bytes = b"{}") -> tuple[Any, Any]:
    ...
    environment = {
        ...
        "telemetry_samples": [{"phase": "fixture"}],
    ...
    independent["identity_receipt_sha256"] = sha(json.dumps(
        independent, sort_keys=True, separators=(",", ":"), ensure_ascii=True, allow_nan=False
    ).encode("ascii"))
    all150 = {
        "schema": "uwfa-sc-v4-all150-source-free-preflight",
        "source_snapshot_root_sha256": source_root,
        "status": "PASS_ALL_150_CPU_CUPY_EXACT_REPEATED",
        "cells": [{"repeated_gpu_run_exact": True, "selector_ordinal": index} for index in range(150)],
        "cell_count": 150,
    ...
    representative = {
        ...
        "telemetry": copy.deepcopy(environment),
...
def build_fixture(experts: int = 2, hidden: int = 3, intermediate: int = 5, *, shared: bool = True) -> tuple[bytes, dict[str, Any], dict[str, Any]]:
    ...
    raw, metrics = codec.build_container(
        ...
        immutable_state=b"source-only-v4-fixture",
...
class PortabilityTests(unittest.TestCase):
    ...
    def test_e250_unequal_shape_shared_tail_descriptor_portability(self) -> None:
        ...
        raw, _diagnostic = codec.build_container(
            ...
            immutable_state=b"e250-unequal-v4", regions=source["regions"],
...
class ScientificPipelineTests(unittest.TestCase):
    ...
    def test_score_receipt_binds_full_geometry_and_independent_decoder(self) -> None:
        ...
        record = {
            "schema": "uwfa-bound-baseline-score-v4",
