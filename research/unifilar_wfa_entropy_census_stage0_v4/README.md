# Universal UWFA-SC v4 — pre-review source-only producer

This directory is a new, source-only successor to the independently blocked v3
producer. The v3 tree and its audit remain unchanged. This v4 tree is
...
The only measured object is one complete `UWFCV4` byte string:
...
bind before fitting starts. Cross-source equality uses only structural geometry;
the source and each control bind their source-dependent arithmetic lengths to
their own artifact, score, reconstruction, and full-geometry receipt. Each
...
- an externally pinned v4 source audit;
