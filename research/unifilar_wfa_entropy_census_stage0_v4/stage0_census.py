#!/usr/bin/env python3
"""Authenticated-snapshot producer core for universal UWFA-SC v4.
...
PRODUCER_ABI = "uwfa-sc-v4-authenticated-snapshot-producer-abi-1"
...
def _validate_environment_identity(protocol: Any, environment: Any, independent: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if independent_row["schema"] != "uwfa-sc-v4-independent-gpu-identity" or independent_row["status"] != "PASS_INDEPENDENT_GPU_IDENTITY":
    ...
    statistics_row = row["statistics"]
    if not isinstance(statistics_row, dict) or int(statistics_row.get("telemetry_samples", 0)) <= 0:
        raise ValueError("CUDA telemetry samples absent")
    if int(statistics_row.get("peak_process_tree_rss_bytes", 0)) <= 0 or int(statistics_row.get("total_vram_bytes", 0)) <= 0:
        raise ValueError("CUDA fatal host/device telemetry missing")
    return dict(row)
...
def validate_source_preflight(common: Any, protocol: Any, evidence: Any, bindings: BoundEvidence) -> dict[str, Any]:
    ...
    if all150.get("schema") != "uwfa-sc-v4-all150-source-free-preflight" or all150.get("status") != "PASS_ALL_150_CPU_CUPY_EXACT_REPEATED":
        raise ValueError("all150 preflight schema/status")
    if all150.get("source_snapshot_root_sha256") != bindings.source_snapshot_root_sha256:
        raise ValueError("all150 source snapshot root")
    if all150.get("cell_count") != 150 or not isinstance(all150.get("cells"), list) or len(all150["cells"]) != 150:
        raise ValueError("all150 exact candidate coverage")
    if not all(row.get("repeated_gpu_run_exact") is True for row in all150["cells"]):
        raise ValueError("all150 repeated CPU/CuPy equality")
    if representative.get("schema") != "uwfa-sc-v4-representative-source-free-preflight" or representative.get("status") != "PASS_REPRESENTATIVE_SOURCE_FREE_OUTER_FOLD":
        raise ValueError("representative preflight schema/status")
    if representative.get("source_snapshot_root_sha256") != bindings.source_snapshot_root_sha256:
        raise ValueError("representative source snapshot root")
    fixture = representative.get("fixture")
    outer = representative.get("outer_fold")
    runtime = representative.get("runtime_projection")
    if not isinstance(fixture, dict) or (fixture.get("streams"), fixture.get("semantic_owners"), fixture.get("private_streams"), fixture.get("shared_tail_streams")) != (15, 6, 12, 3):
        raise ValueError("representative fixture geometry")
    if not isinstance(outer, dict) or outer.get("all_150_candidates_fit_and_scored") is not True or outer.get("literal_container_parse_decode_reencode_rebuild") is not True:
        raise ValueError("representative complete pipeline")
    if not isinstance(runtime, dict) or runtime.get("passes") is not True:
        raise ValueError("representative runtime budget")
    for name in ("model_h2d_bytes_nonzero", "d2h_bytes_nonzero", "peak_host_ram_recorded", "peak_vram_recorded"):
        if representative.get(name) is not True:
            raise ValueError(f"representative telemetry gate: {name}")
    all150_environment = _validate_environment_identity(protocol, all150.get("environment"), independent)
    representative_environment = _validate_environment_identity(protocol, representative.get("telemetry"), independent)
...
def representative_outer_fold_benchmark(
    ...
    semantic_packet = semantic_codec.build_semantic_packet(shapes, b"representative-outer-fold-v4")
    ...
    for candidate in common.candidate_bank():
        ...
        choices.append((validation_score(common, lengths, candidate), candidate.selector_ordinal, candidate))
    ...
    if container_codec.canonical_rebuild(common, semantic_codec, parsed) != container:
...
def _split_digest(protocol: Any, identity: tuple[int, int], row: Mapping[str, Any]) -> bytes:
    digest = protocol.length_prefixed_digest(
        ...
        domain=b"UWFA-SC-V4-NESTED-SPLIT-2026-09-02\x00",
    )
        ...
        for context, positions in groups.items():
            ranked_sources = sorted(
                ...
                key=lambda position: protocol.length_prefixed_digest(
                    ...
                    domain=b"UWFA-SC-V4-WITHIN-CONTEXT-PERMUTATION\x00",
                ),
            )
    ...
    for row in panel["streams"]:
        ...
        ranked = sorted(
            ...
            key=lambda item: protocol.length_prefixed_digest(
                ...
                domain=b"UWFA-SC-V4-MULTISCALE-CHUNK-SHUFFLE\x00",
            ),
        )
...
def source_phase(
    *,
    ...
    if not runtime_projection["primary_exact_identity_estimable"]:
        return {
            "schema": "uwfa-sc-v4-source-phase-result",
...
def controls_phase(
    *,
    ...
    source_full_geometry = str(source_result["source_full_geometry_sha256"])
    source_structural_geometry = str(source_result["source_structural_geometry_sha256"])
    ...
    for expected_seed, item in zip(common.CONTROL_SEEDS, controls, strict=True):
        ...
        if (
            ...
            raise ValueError("control pipeline/geometry BoundEvidence mismatch")
        score = protocol.validate_score_receipt(
            score,
            artifact_sha256=artifact_sha,
    ...
    return {
        "schema": "uwfa-sc-v4-control-phase-result",
