...
def make_fixture(
    ...
    *,
    ...
    semantic_packet = semantic_codec.build_semantic_packet(shapes, b"source-free-universal-fixture-v4")
    ...
    for ordinal, raw in enumerate(raw_specs):
        ...
        spec = codec.StreamSpec(
            ...
            source_digest=common.selected_decision_triplet_sha256(
                bytes(bits), bytes(levels), struct.pack(f"<{len(base)}H", *base)
            ),
            profile_q=0,
            decoder_scale=1.0,
            role=raw["role"],
...
def make_unequal_shape_e250_fixture(common: Any, codec: Any, semantic_codec: Any) -> dict[str, Any]:
    """Source-free E250 fixture retained from the v3 independent audit.

    The shared gate tail deliberately gives owner 0 one scalar and owner 249
    two scalars.  This catches equal-share source attribution, fixed-shape
    adapters, truncated owner sets, and non-generic routed reconstruction.
    """
    experts = 250
    shapes = tuple(
        semantic_codec.ExpertShape(index, 1 + index % 3, 2 + index % 4)
        for index in range(experts)
    )
    semantic_packet = semantic_codec.build_semantic_packet(shapes, b"source-free-unequal-shape-e250-v4")
    candidate = common.Candidate("suffix", 2, 32)
    frequencies = [32768] * common.model_frequency_count(candidate)
    model_packet = common.serialize_model(candidate, frequencies)
    ...
    for expert, shape in enumerate(shapes):
        matrix_weights = shape.matrix_weights
        for role in semantic_codec.ROLES:
            prefix = matrix_weights - tail.get((expert, role), 0)
            raw_specs.append((role, ((expert, 0, prefix),)))
    raw_specs.append((
        "gate",
        (
    ...
    stream_specs: list[tuple[bytes, Any]] = []
    ...
    for ordinal, (role, contribution_rows) in enumerate(raw_specs):
        ...
        stream_specs.append((owner_set, codec.StreamSpec(
            ordinal=ordinal,
            symbols=symbols,
            logical_bits=logical_bits,
            payload=payload,
        ...
        references.append({"bits": bits, "levels": levels, "base": base})
    grouped: dict[bytes, list[Any]] = defaultdict(list)
    for owner_set, spec in stream_specs:
        grouped[owner_set].append(spec)
    owner_sets = sorted(
    ...
    for (_owner_set, spec), reference in zip(stream_specs, references, strict=True):
        ...
        for contribution in spec.owner_contributions:
            ...
            for local in range(contribution.weight_count):
                position = contribution.source_offset + local
                if target[position] is not None:
                    raise ValueError("unequal-shape fixture reconstruction overlap")
                target[position] = _scalar_bytes(bits[local % len(bits)], spec.ordinal, position)
    digest = hashlib.sha256()
    for expert in range(experts):
        for role in semantic_codec.ROLES:
            values = matrices[(expert, role)]
            if any(value is None for value in values):
                raise ValueError("unequal-shape fixture reconstruction hole")
            digest.update(b"".join(values))
    return {
        "experts": experts,
        ...
        "frequencies": frequencies,
        "regions": regions,
        "reference_streams": tuple(references),
        "reconstruction_sha256": digest.hexdigest(),
        "expected_streams": 751,
        "expected_regions": 251,
    }
...
class FixtureRoutedDecoder:
    ...
    def decode_expert(self, route: dict[str, Any]) -> dict[str, Any]:
        ...
        for row in route["rows"]:
            ...
            if self.common.selected_decision_triplet_sha256(
                bytes(bits), bytes(levels), struct.pack(f"<{len(base)}H", *base)
            ) != row["source_digest"]:
...
def decode_and_reencode(common: Any, parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in parsed["directory"]:
        ...
        if common.selected_decision_triplet_sha256(
            bytes(bits), bytes(levels), struct.pack(f"<{len(base)}H", *base)
        ) != row["source_digest"]:
