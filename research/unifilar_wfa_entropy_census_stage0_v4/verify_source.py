#!/usr/bin/env python3
"""Native verifier for a post-review sealed UWFA-SC v4 source package."""
...
POST_FREEZE_REQUIREMENTS = [
    "EXTERNAL_PINNED_V4_SOURCE_AUDIT",
...
def verify_package(package: Path) -> dict[str, Any]:
    ...
    require(manifest["schema"] == "unifilar-wfa-source-manifest-v4", "manifest schema")
    ...
    require(design.get("schema") == "unifilar-wfa-entropy-census-design-v4", "design schema")
    ...
    common = module_from_snapshot("uwfa_verify_common_v4", snapshots["uwfa_common.py"])
    ...
    require(model.startswith(b"UWFAV4\x00\x00"), "v4 serialized model")
    ...
    for token in ("UWFCV4", "UWFDIR4", "UWFREG4", "UWFFRM4", "VERSION = 4", "DIRECTORY_RECORD_BYTES = 256", "REGION_HEADER_BYTES = 256", "FRAME_HEADER_BYTES = 256", "def routed_read_expert", "def physical_metrics", "def posterior_diagnostic_handoff", "decoded_selected_decision_triplet_sha256"):
    ...
    for token in ("class SourcePreflightEvidence", "def validate_source_preflight", "source_full_geometry_sha256", "source_structural_geometry_sha256", "passes_pre_fit_resource_budget", "policy=\"coordinate_disjoint\""):
        require(token in stage_text, f"v4 source gate: {token}")
    ...
    for token in ("class RetainedOutputParent", "class CompletionLastOutput", "renameat2", "RENAME_NOREPLACE"):
        require(token in common_text, f"descriptor-relative publication contract: {token}")

    return {
        "schema": "unifilar-wfa-source-verification-v4",
