#!/usr/bin/env python3
"""Hostile source-only UWFA-SC v7 producer tests.
...
common = load("uwfa_v7_test_common", "uwfa_common.py")
protocol = load("uwfa_v7_test_protocol", "protocol.py")
semantic = load("uwfa_v7_test_semantic", "universal_adapter.py")
codec = load("uwfa_v7_test_container", "container_codec.py")
fixture = load("uwfa_v7_test_fixture", "fixture_portability.py")
strata = load("uwfa_v7_test_strata", "strata_sc_adapter.py")
stage = load("uwfa_v7_test_stage", "stage0_census.py")
cupy_backend = load("uwfa_v7_test_cupy", "cupy_backend.py")
dispatcher = load("uwfa_v7_test_dispatcher", "dispatcher_contract.py")
envelope = load("uwfa_v7_test_envelope", "result_envelope.py")
...
def build_fixture(experts: int = 2, hidden: int = 3, intermediate: int = 5, *, shared: bool = True) -> tuple[bytes, dict[str, Any], dict[str, Any]]:
    ...
    raw, metrics = codec.build_container(
        ...
        immutable_state=b"source-only-v7-fixture",
...
class LiteralContainerTests(unittest.TestCase):
    ...
    def test_shared_measure_only_grammar_equals_actual_serialized_bytes(self) -> None:
        measured = codec.measure_literal_container_layout(
            ...
            immutable_state_bytes=len(b"source-only-v7-fixture"),
...
class PortabilityTests(unittest.TestCase):
    ...
    def test_e250_unequal_shape_shared_tail_descriptor_portability(self) -> None:
        ...
        raw, _diagnostic = codec.build_container(
            ...
            immutable_state=b"e250-unequal-v7", regions=source["regions"],
...
def scientific_panel() -> dict[str, Any]:
    ...
    return {
        ...
        "immutable_state": b"",
        "semantic_packet": semantic.build_semantic_packet(shapes, b"scientific-panel-v7"),
...
class ScientificPipelineTests(unittest.TestCase):
    ...
    def test_legal_single_expert_panel_fails_safe_before_any_fit(self) -> None:
        ...
        backend = mock.Mock()
        panel["semantic_packet"] = semantic.build_semantic_packet((semantic.ExpertShape(0, 1, 1),), b"scientific-single-v7")
        result = stage.nested_holdout(common, protocol, codec, backend, object(), panel)
        self.assertEqual(result["status"], "HOLD_NONESTIMABLE_DEPENDENCE_COMPONENTS")
        self.assertFalse(result["estimable"])
        backend.fit_counts.assert_not_called()
    ...
    def test_score_receipt_binds_full_geometry_and_independent_decoder(self) -> None:
        ...
        record = {
            "schema": "uwfa-bound-baseline-score-v7",
