#!/usr/bin/env python3
"""Strict bounded records for the source-only UWFA-SC v7 producer.
...
def canonical_gpu_uuid(value: Any, label: str) -> str:
    if not isinstance(value, str) or GPU_UUID.fullmatch(value) is None:
        raise ValueError(f"{label} must be canonical GPU UUID text")
    if value == "GPU-00000000-0000-0000-0000-000000000000":
        raise ValueError(f"{label} must not be the all-zero placeholder")
    return value


def canonical_pci_bus_id(value: Any, label: str) -> str:
    if not isinstance(value, str) or PCI_BUS_ID.fullmatch(value) is None:
        raise ValueError(f"{label} must be canonical PCI bus text")
    return value
...
def validate_score_receipt(
    ...
    *,
    ...
    original_source_panel_sha256: str,
    ...
    if row["schema"] != "uwfa-bound-baseline-score-v7" or row["status"] != "PASS_INDEPENDENT_BASELINE_SCORE":
    ...
    if sha256_hex(row["original_source_panel_sha256"], "source panel") != sha256_hex(original_source_panel_sha256, "expected source panel"):
        raise ValueError("score/source full-geometry binding mismatch")
    if sha256_hex(row["independent_decoder_source_sha256"], "decoder source") != sha256_hex(independent_decoder_source_sha256, "expected decoder source"):
        raise ValueError("score/independent-decoder binding mismatch")
...
def structural_panel_geometry(panel: Mapping[str, Any]) -> dict[str, Any]:
    """Return only cross-source invariants, never arithmetic payload lengths."""
    full = panel_geometry(panel)
    return {
        "weights": full["weights"],
        "experts": full["experts"],
        "immutable_state_bytes": full["immutable_state_bytes"],
        "semantic_identities": full["semantic_identities"],
        "expert_shapes": full["expert_shapes"],
        "streams": [
            {key: value for key, value in row.items() if key not in {"baseline_payload_bytes", "baseline_logical_bits"}}
            for row in full["streams"]
        ],
    }


def structural_geometry_sha256(common: Any, panel: Mapping[str, Any]) -> str:
    return hashlib.sha256(common.canonical_json(structural_panel_geometry(panel))).hexdigest()
...
def validate_control_binding(
    ...
    *,
    ...
    symmetric_closure: Mapping[str, str],
) -> dict[str, Any]:
    closure_fields = (
        "baseline_plan_sha256", "universal_decoder_sha256",
        "producer_manifest_sha256", "audit_bootstrap_sha256",
        "extraction_program_sha256", "universal_adapter_sha256",
        "source_snapshot_root_sha256", "source_preflight_receipt_sha256",
    )
    if not isinstance(symmetric_closure, dict) or set(symmetric_closure) != set(closure_fields) | {"pipeline_sha256"}:
        raise ValueError("symmetric source/control closure schema")
    if sha256_hex(symmetric_closure["pipeline_sha256"], "symmetric pipeline") != sha256_hex(source_pipeline_sha256, "source pipeline"):
        raise ValueError("symmetric closure pipeline mismatch")
    for name in closure_fields:
        sha256_hex(symmetric_closure[name], f"symmetric closure {name}")
    row = strict_fields(
        record,
        required=(
            "schema", "seed", "source_artifact_sha256",
            "source_full_geometry_sha256", "source_structural_geometry_sha256",
            "pipeline_sha256", "generator_source_sha256",
            "moment_match_receipt_sha256", "control_artifact_sha256",
    ...
    if row["schema"] != "uwfa-matched-gaussian-control-binding-v7":
    ...
    expected = {
        ...
        "control_full_geometry_sha256": control_full_geometry_sha256,
        "control_structural_geometry_sha256": control_structural_geometry_sha256,
        **{name: symmetric_closure[name] for name in closure_fields},
    }
