#!/usr/bin/env python3
"""Authenticated-snapshot producer core for universal UWFA-SC v7.
...
PRODUCER_ABI = "uwfa-sc-v7-authenticated-snapshot-producer-abi-1"
...
def _validate_environment_identity(protocol: Any, environment: Any, independent: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if independent_row["schema"] != "uwfa-sc-v7-independent-gpu-identity" or independent_row["status"] != "PASS_INDEPENDENT_GPU_IDENTITY":
...
def _validate_all150_receipt(common: Any, protocol: Any, value: Any, bindings: BoundEvidence, independent: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if row["schema"] != "uwfa-sc-v7-all150-source-free-preflight" or row["status"] != "PASS_ALL_150_CPU_CUPY_EXACT_REPEATED":
...
def _validate_representative_receipt(common: Any, protocol: Any, value: Any, bindings: BoundEvidence, independent: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if row["schema"] != "uwfa-sc-v7-representative-source-free-preflight" or row["status"] != "PASS_REPRESENTATIVE_SOURCE_FREE_OUTER_FOLD":
...
def literal_validation_score(
    ...
    """Charge the exact frozen literal container for an inner split.

    Validation streams use the candidate's exact arithmetic lengths. Every
    other stream retains the authenticated baseline payload geometry, which
    is fixed across candidates and reveals no held-out candidate score. The
    shared measure-only grammar is also the serializer's layout authority, so
    frame, directory, model, 64-byte/4-KiB alignment and rate-floor padding
    cannot diverge without the real builder failing its own assertion.
    """
...
def gpu_preflight_all_150(common: Any, backend: Any, source_snapshot_root_sha256: str) -> dict[str, Any]:
    ...
    return {
        "schema": "uwfa-sc-v7-all150-source-free-preflight",
...
def representative_outer_fold_benchmark(
    ...
    semantic_packet = semantic_codec.build_semantic_packet(shapes, b"representative-outer-fold-v7")
    ...
    return {
        "schema": "uwfa-sc-v7-representative-source-free-preflight",
        "source_snapshot_root_sha256": source_snapshot_root_sha256.lower(),
...
def _split_digest(protocol: Any, identity: tuple[int, int], row: Mapping[str, Any]) -> bytes:
    digest = protocol.length_prefixed_digest(
        ...
        domain=b"UWFA-SC-V7-NESTED-SPLIT-2026-09-02\x00",
    )
            ...
            ranked = sorted(
                ...
                key=lambda index: (
                    protocol.length_prefixed_digest(
                        ...
                        domain=b"UWFA-SC-V7-DISJOINT-COMPONENT-SPLIT-2026-09-02\x00",
                    ),
        ...
        for context, positions in groups.items():
            ranked_sources = sorted(
                ...
                key=lambda position: protocol.length_prefixed_digest(
                    ...
                    domain=b"UWFA-SC-V7-WITHIN-CONTEXT-PERMUTATION\x00",
                ),
            )
    ...
    for row in panel["streams"]:
        ...
        ranked = sorted(
            ...
            key=lambda item: protocol.length_prefixed_digest(
                ...
                domain=b"UWFA-SC-V7-MULTISCALE-CHUNK-SHUFFLE\x00",
            ),
        )
...
def nested_holdout(
    ...
    *,
    ...
    for plan in valid_plans:
        ...
        literal_baseline_bits = literal_validation_score(
        ...
        literal_saved_bits = literal_baseline_bits - literal_candidate_bits - model_bits
        ...
        outer = {
            ...
            "literal_baseline_hybrid_container_bits": literal_baseline_bits,
            "literal_candidate_hybrid_container_bits": literal_candidate_bits,
            "literal_test_saving_after_model_bits": literal_saved_bits,
    ...
    pooled_saved_bits = sum(float(row["literal_test_saving_after_model_bits"]) for row in fold_rows)
    ...
    if len(fold_rows) > 1:
        for omitted, row in enumerate(fold_rows):
            ...
            kept_bits = pooled_saved_bits - float(row["literal_test_saving_after_model_bits"])
...
def source_phase(
    *,
    ...
    if not runtime_projection["primary_exact_identity_estimable"]:
        return {
            "schema": "uwfa-sc-v7-source-phase-result",
...
def controls_phase(
    *,
    ...
    return {
        "schema": "uwfa-sc-v7-control-phase-result",
