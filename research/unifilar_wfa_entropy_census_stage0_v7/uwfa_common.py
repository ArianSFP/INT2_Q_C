#!/usr/bin/env python3
"""Standard-library contracts and exact CPU reference for UWFA census v7.
...
DESIGN_SCHEMA = "unifilar-wfa-entropy-census-design-v7"
RESULT_SCHEMA = "unifilar-wfa-entropy-census-result-v7"
REVIEW_SCHEMA = "unifilar-wfa-entropy-census-independent-source-review-v7"
STREAM_LOCK_SCHEMA = "unifilar-wfa-canonical-stream-lock-v7"
CONTROL_LOCK_SCHEMA = "unifilar-wfa-gaussian-control-panel-lock-v7"
AUTHORIZATION = "OPEN_AUTHENTICATED_UNIFILAR_WFA_CENSUS_AFTER_INDEPENDENT_SOURCE_REVIEW_V7"
...
SPLIT_HASH_SEED = b"UWFA-V7-NESTED-SPLIT-2026-09-02"
...
def deserialize_model(packet: bytes) -> tuple[Candidate, list[int]]:
    ...
    require(magic == b"UWFAV7\x00\x00" and version == 3, "model magic/version")
...
def parent_commit_marker_name(final_name: str) -> str:
    require(
        isinstance(final_name, str) and final_name == Path(final_name).name
        and final_name not in {"", ".", ".."} and "/" not in final_name and "\\" not in final_name,
        "output final name",
    )
    digest = sha256_bytes(b"UWFA-V7-COMMIT-NAME\x00" + final_name.encode("utf-8"))
    return f".uwfa-publish-v7-{digest}.json"


def committed_directory_root(source_manifest_sha256: str, members: list[dict[str, Any]]) -> str:
    require(isinstance(source_manifest_sha256, str) and len(source_manifest_sha256) == 64, "source root digest")
    try:
        require(len(bytes.fromhex(source_manifest_sha256)) == 32, "source root digest width")
    except ValueError as exc:
        raise ContractError("source root digest encoding") from exc
    canonical_members = sorted((dict(row) for row in members), key=lambda row: row["name"].encode("utf-8"))
    body = canonical_json({
        "source_manifest_sha256": source_manifest_sha256,
        "members": canonical_members,
    })
    return sha256_bytes(b"UWFA-V7-HELD-DIRECTORY-ROOT\x00" + body)


def _open_held_commit_authority_file(parent_fd: int, anchor_name: str) -> tuple[int, str | None]:
    """Open a held inode in the retained output parent.

    A named temporary file would recreate the staging-name race.  Linux
    ``O_TMPFILE`` makes the file descriptor itself the sole authority until
    ``linkat(AT_EMPTY_PATH)`` publishes that exact inode.  Some otherwise
    suitable filesystems (including the RunPod overlay used for development)
    reject ``O_TMPFILE``.  There the fallback name is only a creation anchor:
    the final marker is still linked from the held fd with ``AT_EMPTY_PATH``;
    the anchor path is never a publication source and is removed only when it
    still resolves to the held inode.
    """
    require(os.name == "posix", "unnamed commit publication requires POSIX")
    require(anchor_name == Path(anchor_name).name and anchor_name not in {"", ".", ".."}, "commit anchor name")
    ...
    if tmpfile:
        ...
        try:
            ...
            import errno
            if exc.errno not in {errno.EOPNOTSUPP, errno.ENOTSUP, errno.EINVAL}:
                raise ContractError(f"O_TMPFILE commit authority unavailable: {exc}") from exc
        else:
            info = os.fstat(fd)
            if not stat.S_ISREG(info.st_mode) or int(info.st_nlink) != 0:
                os.close(fd)
                raise ContractError("unnamed commit authority is not an unlinked regular inode")
            return fd, None
    flags = (
    ...
    try:
        fd = os.open(anchor_name, flags, 0o600, dir_fd=parent_fd)
    except OSError as exc:
        raise ContractError(f"held commit-anchor creation failed: {exc}") from exc
    info = os.fstat(fd)
    if not stat.S_ISREG(info.st_mode) or int(info.st_nlink) != 1:
        os.close(fd)
        raise ContractError("held commit anchor is not a singly linked regular inode")
    return fd, anchor_name
...
def _linkat_syscall(
    ...
    """Execute one ``linkat`` and return zero or its exact errno.

    Keeping the two descriptor-addressed publication branches behind this
    deliberately tiny seam lets the source-only hostile suite force both
    ``AT_EMPTY_PATH`` and ``/proc/self/fd`` paths.  Production still invokes
    the libc syscall directly and treats every nonzero return as an errno.
    """
    import ctypes
    libc = ctypes.CDLL(None, use_errno=True)
    linkat = getattr(libc, "linkat", None)
    if linkat is None:
        raise ContractError("linkat unavailable; commit publication refused")
    ...
    return 0 if result == 0 else int(ctypes.get_errno())


def _link_held_unnamed_file_noreplace(source_fd: int, parent_fd: int, destination: str) -> str:
    """Atomically link the exact held ``O_TMPFILE`` inode under ``destination``."""
    require(destination == Path(destination).name and destination not in {"", ".", ".."}, "commit marker name")
    if os.name != "posix":
        raise ContractError("fd-addressed commit publication requires POSIX")
    at_empty_path = 0x1000
    destination_bytes = os.fsencode(destination)
    error = _linkat_syscall(source_fd, b"", parent_fd, destination_bytes, at_empty_path)
    if error != 0:
        # Some containers omit CAP_DAC_READ_SEARCH, making AT_EMPTY_PATH
        # return ENOENT even for our own held fd.  /proc/self/fd is an exact
        # kernel-owned reference to that fd, not the mutable creation anchor.
        # AT_SYMLINK_FOLLOW therefore remains descriptor-addressed and the
        # caller verifies the destination inode immediately afterwards.
        import errno
        if error in {errno.ENOENT, errno.EPERM, errno.EACCES}:
            at_fdcwd = -100
            at_symlink_follow = 0x400
            proc_fd = os.fsencode(f"/proc/self/fd/{source_fd}")
            error = _linkat_syscall(at_fdcwd, proc_fd, parent_fd, destination_bytes, at_symlink_follow)
            if error == 0:
                return "PROC_SELF_FD_AT_SYMLINK_FOLLOW"
        if error != 0:
            raise OSError(error, os.strerror(error), destination)
...
def _write_all_to_fd(fd: int, data: bytes) -> None:
    ...
    while written < len(view):
        ...
        require(count > 0, "short commit-marker write")
        written += count
...
class CompletionLastOutput:
    ...
    def __init__(self, parent: RetainedOutputParent, final_name: str, transaction_id: str):
        ...
        self.commit_marker_name = parent_commit_marker_name(final_name)
        anchor_suffix = sha256_bytes(b"UWFA-V7-COMMIT-ANCHOR\x00" + final_name.encode("utf-8"))[:16]
        self.commit_anchor_name = f".uwfa-commit-anchor-v7-{self.transaction_id}-{anchor_suffix}.incomplete"
        self.completed = False
        self.published = False
        self.durable = False
        self.named_move_completed = False
        self.commit_marker_published = False
    ...
    def __enter__(self) -> "CompletionLastOutput":
        ...
        for name in (self.final_name, self.staging_name, self.commit_marker_name, self.commit_anchor_name):
            try:
        ...
        self.write_new("RUN_STATE.json", pretty_json({
            "schema": "unifilar-wfa-run-state-v7",
            "complete": False,
    ...
    def _verify_final_name_binding(self) -> os.stat_result:
        require(self.dir_fd is not None, "output descriptor absent")
        self.parent.verify_stable()
        held = os.fstat(self.dir_fd)
        require(stat.S_ISDIR(held.st_mode), "held output directory ceased to be a directory")
        named = os.stat(self.final_name, dir_fd=self.parent.fd, follow_symlinks=False)
        require(stat.S_ISDIR(named.st_mode), "published output name is not a directory")
        require(
            (int(named.st_dev), int(named.st_ino)) == (int(held.st_dev), int(held.st_ino)),
            "published output name is not the retained staging directory",
        )
        return held

    def _verify_staging_name_binding(self) -> os.stat_result:
        require(self.dir_fd is not None, "output descriptor absent")
        self.parent.verify_stable()
        held = os.fstat(self.dir_fd)
        named = os.stat(self.staging_name, dir_fd=self.parent.fd, follow_symlinks=False)
        require(stat.S_ISDIR(named.st_mode), "staging output name is not a directory")
        require(
            (int(named.st_dev), int(named.st_ino)) == (int(held.st_dev), int(held.st_ino)),
            "staging output name is not the retained staging directory",
        )
        return held

    def _verify_named_commit_binding(self, marker_fd: int) -> os.stat_result:
        held = os.fstat(marker_fd)
        require(stat.S_ISREG(held.st_mode), "held commit marker ceased to be regular")
        named = os.stat(self.commit_marker_name, dir_fd=self.parent.fd, follow_symlinks=False)
        require(stat.S_ISREG(named.st_mode), "named commit marker is not regular")
        require(
            (int(named.st_dev), int(named.st_ino)) == (int(held.st_dev), int(held.st_ino)),
            "named commit marker is not the held unnamed inode",
        )
        return held

    def _verify_held_commit_bytes(
        self,
        marker_fd: int,
        marker_bytes: bytes,
        *,
        ...
        """Re-read the exact held marker inode and bind content plus links."""
        require(type(marker_bytes) is bytes and marker_bytes, "commit marker bytes")
        require(type(expected_links) is int and expected_links in {0, 1}, "commit marker link count")
        before = os.fstat(marker_fd)
        require(stat.S_ISREG(before.st_mode), "held commit marker ceased to be regular")
        require(int(before.st_size) == len(marker_bytes), "held commit marker size changed")
        require(int(before.st_nlink) == expected_links, "held commit marker link count changed")
        os.lseek(marker_fd, 0, os.SEEK_SET)
        digest = hashlib.sha256()
        observed = bytearray()
        while len(observed) < len(marker_bytes):
            chunk = os.read(marker_fd, min(1 << 20, len(marker_bytes) - len(observed)))
            require(chunk, "short held commit-marker read")
            observed.extend(chunk)
            digest.update(chunk)
        require(os.read(marker_fd, 1) == b"", "held commit marker trailing bytes")
        after = os.fstat(marker_fd)
        identity = lambda info: (
            int(info.st_dev), int(info.st_ino), int(info.st_mode), int(info.st_size),
            int(info.st_nlink), int(info.st_mtime_ns), int(info.st_ctime_ns),
        )
        require(identity(before) == identity(after), "held commit marker changed while hashing")
        require(bytes(observed) == marker_bytes, "held commit-marker byte mismatch")
        require(digest.hexdigest() == sha256_bytes(marker_bytes), "held commit-marker digest mismatch")
        return after

    def _parent_commit_record(
        self,
        *,
        ...
        committed_members = sorted(
            (dict(row) for row in self.members),
            key=lambda row: row["name"].encode("utf-8"),
        )
        return seal_record(
            {
                "schema": "unifilar-wfa-parent-commit-v7",
                "status": "PARENT_MARKER_COMMITTED",
                "final_name": self.final_name,
                "transaction_id": self.transaction_id,
                "output_parent_authority_sha256": self.parent.authority_sha256,
                "parent_device": int(parent_info.st_dev),
                "parent_inode": int(parent_info.st_ino),
                "final_directory_device": int(directory_info.st_dev),
                "final_directory_inode": int(directory_info.st_ino),
                "commit_marker_device": int(marker_info.st_dev),
                "commit_marker_inode": int(marker_info.st_ino),
                "source_manifest_sha256": source_manifest_sha256,
                "members": committed_members,
                "directory_root_sha256": committed_directory_root(source_manifest_sha256, committed_members),
                "completion_sha256": completion_sha256,
            },
            "parent_commit_sha256",
        )

    def _verify_exact_staging_members(self, expected: list[dict[str, Any]]) -> None:
        """Re-enumerate and re-hash the held staging directory fail-closed.

        Caller-maintained metadata is never publication authority.  Every
        declared member is reopened relative to the retained staging
        descriptor with no-follow semantics, required to be a stable regular
        file, and rehashed.  Any missing, duplicate, mutated, nonregular, or
        undeclared member aborts before publication.
        """
        require(self.dir_fd is not None, "output descriptor absent")
        require(isinstance(expected, list) and expected, "nonempty staging commitment")
        expected_by_name: dict[str, dict[str, Any]] = {}
        for row in expected:
            require(isinstance(row, dict) and set(row) == {"name", "bytes", "sha256"}, "staging commitment row")
            name = row["name"]
            require(
                isinstance(name, str) and name == Path(name).name
                and name not in {"", ".", ".."} and "/" not in name and "\\" not in name,
                "staging commitment name",
            )
            require(name not in expected_by_name, "duplicate staging commitment name")
            require(type(row["bytes"]) is int and row["bytes"] >= 0, "staging commitment bytes")
            require(isinstance(row["sha256"], str) and len(row["sha256"]) == 64, "staging commitment digest")
            try:
                raw_digest = bytes.fromhex(row["sha256"])
            except ValueError as exc:
                raise ContractError("staging commitment digest encoding") from exc
            require(len(raw_digest) == 32, "staging commitment digest width")
        ...
        require(
            sorted(actual_names) == sorted(expected_by_name)
            and len(actual_names) == len(set(actual_names)),
            "staging directory has missing or undeclared members",
        )
        ...
        for name in sorted(expected_by_name):
            ...
            try:
                before = os.fstat(fd)
                require(stat.S_ISREG(before.st_mode), f"staging member is not regular: {name}")
                require(before.st_size == row["bytes"], f"staging member size changed: {name}")
                digest = hashlib.sha256()
                observed_bytes = 0
                while True:
                ...
                require(identity(before) == identity(after), f"staging member changed while hashing: {name}")
                require(observed_bytes == row["bytes"], f"staging member short read: {name}")
                require(digest.hexdigest() == row["sha256"], f"staging member digest changed: {name}")
            finally:
                os.close(fd)
        os.fsync(self.dir_fd)
    ...
    def complete(self, members: list[dict[str, Any]], source_manifest_sha256: str) -> dict[str, Any]:
        ...
        # Authority is the held directory bytes, never the caller's in-memory
        # metadata.  Verify immediately before constructing COMPLETE.
        self._verify_exact_staging_members(list(members))
        record = seal_record(
            {
                "schema": "unifilar-wfa-completion-v7",
        ...
        self.completed = True
        require(self.dir_fd is not None, "output descriptor absent")
        # Catch an undeclared/mutated member racing the COMPLETE write itself.
        # This second pass includes COMPLETE and occurs immediately before the
        # no-replace directory rename.
        self._verify_exact_staging_members(list(self.members))
        os.fsync(self.dir_fd)
        self.parent.verify_stable()
        self._verify_staging_name_binding()
        _rename_directory_noreplace(self.parent.fd, self.staging_name, self.final_name)
        self.named_move_completed = True

        # A successful name-based rename is not completion authority.  Bind
        # the final name back to the exact retained staging descriptor, then
        # re-enumerate/re-hash through that descriptor after the move.
        directory_info = self._verify_final_name_binding()
        self._verify_exact_staging_members(list(self.members))
        os.fsync(self.parent.fd)

        marker_fd, marker_anchor_name = _open_held_commit_authority_file(
            self.parent.fd, self.commit_anchor_name
        )
        try:
            marker_info = os.fstat(marker_fd)
            marker_record = self._parent_commit_record(
                source_manifest_sha256=source_manifest_sha256,
                directory_info=directory_info,
                marker_info=marker_info,
                completion_sha256=str(record["completion_sha256"]),
            )
            marker_bytes = pretty_json(marker_record)
            _write_all_to_fd(marker_fd, marker_bytes)
            os.fchmod(marker_fd, 0o400)
            os.fsync(marker_fd)
            after_write = os.fstat(marker_fd)
            expected_initial_links = 0 if marker_anchor_name is None else 1
            require(
                (int(after_write.st_dev), int(after_write.st_ino), int(after_write.st_size), int(after_write.st_nlink))
                == (int(marker_info.st_dev), int(marker_info.st_ino), len(marker_bytes), expected_initial_links),
                "held commit marker changed before publication",
            )
            self._verify_held_commit_bytes(
                marker_fd, marker_bytes, expected_links=expected_initial_links
            )

            # Repeat both bindings immediately before the fd-addressed link.
            self._verify_final_name_binding()
            self._verify_exact_staging_members(list(self.members))
            self.parent.verify_stable()
            link_method = _link_held_unnamed_file_noreplace(
                marker_fd, self.parent.fd, self.commit_marker_name
            )
            require(
                link_method in {"AT_EMPTY_PATH", "PROC_SELF_FD_AT_SYMLINK_FOLLOW"},
                "unknown commit marker link method",
            )
            self.commit_marker_published = True
            if marker_anchor_name is not None:
                try:
                    anchor_info = os.stat(marker_anchor_name, dir_fd=self.parent.fd, follow_symlinks=False)
                except FileNotFoundError:
                    pass
                else:
                    if (
                        stat.S_ISREG(anchor_info.st_mode)
                        and (int(anchor_info.st_dev), int(anchor_info.st_ino))
                        == (int(marker_info.st_dev), int(marker_info.st_ino))
                    ):
                        os.unlink(marker_anchor_name, dir_fd=self.parent.fd)

            # The final parent marker must now be the sole link.  Re-read and
            # re-hash through the retained fd after anchor cleanup; a content
            # write, copied/replaced name, or extra hardlink is never accepted
            # merely because the pre-link bytes were valid.
            self._verify_named_commit_binding(marker_fd)
            self._verify_held_commit_bytes(marker_fd, marker_bytes, expected_links=1)

            # A race after the link can never silently become authoritative:
            # prove both published names still resolve to the held inodes and
            # the committed member bytes still match before declaring success.
            self._verify_named_commit_binding(marker_fd)
            self._verify_final_name_binding()
            self._verify_exact_staging_members(list(self.members))
            self._verify_held_commit_bytes(marker_fd, marker_bytes, expected_links=1)
            ...
            self._verify_final_name_binding()
            return {
                "completion_member": metadata,
                "parent_commit_marker": {
                    "name": self.commit_marker_name,
                    "bytes": len(marker_bytes),
                    "sha256": sha256_bytes(marker_bytes),
                    "parent_commit_sha256": marker_record["parent_commit_sha256"],
                },
            }
        finally:
            os.close(marker_fd)
    ...
    def __exit__(self, exc_type: Any, exc: Any, traceback: Any) -> None:
        # Incomplete staging/final directories remain fail-closed and are
        # never resumed or deleted here.  A moved directory is complete only
        # when an independently verified parent marker exists and binds it.
