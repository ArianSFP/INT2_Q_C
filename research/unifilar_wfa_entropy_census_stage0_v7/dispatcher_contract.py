...
def authenticate_reference_closure(
    *,
    ...
    try:
        ...
        _require(manifest.get("schema") == "unifilar-wfa-source-manifest-v7", "manifest schema")
        _require(manifest.get("status") == "SEALED_SOURCE_ONLY_NO_PAYLOAD_AUTHORITY", "manifest status")
        _require(review.get("schema") == "unifilar-wfa-entropy-census-independent-source-review-v7", "review schema")
