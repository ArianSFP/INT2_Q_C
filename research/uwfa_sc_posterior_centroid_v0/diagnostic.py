#!/usr/bin/env python3
"""Run the bounded UWFA-SC continuous posterior-centroid v0 diagnostic.

This runner is inert on import.  Execution requires an explicit authorization,
a completed v9 result, the exact result-bound decoder sources, and a generic
hash-authorized SwiGLU BF16 score manifest.  It never edits v8 or v9.
"""
...
PACKAGE_MANIFEST_SCHEMA = "uwfa-sc-posterior-centroid-source-manifest-v0"
RESULT_SCHEMA = "uwfa-sc-posterior-centroid-result-v0"
...
def run(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    core = _load_authenticated_owned_module(
        ...
        private_name="uwfa_pc_v0_authenticated_core",
    )
    bridge = _load_authenticated_owned_module(
        package_closure,
        member_name="result_bridge.py",
        private_name="uwfa_pc_v0_authenticated_bridge",
    )

    # Literal publication and decoder source closure are authenticated before
    # importing NumPy/CuPy or opening any original source matrix.
    ...
    completion = {
        "schema": "uwfa-sc-posterior-centroid-completion-v0",
    ...
    return {
        "schema": "uwfa-sc-posterior-centroid-launch-summary-v0",
        "status": result_record["status"],
        "positive_claim_authority": False,
...
def main() -> int:
    try:
        ...
        print(f"posterior-centroid-v0 failed: {error}", file=sys.stderr)
        return 1
