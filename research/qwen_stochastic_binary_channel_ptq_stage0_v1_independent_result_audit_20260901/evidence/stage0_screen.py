#!/usr/bin/env python3
"""CuPy source-panel stage-0 oracle for QSB-PTQ-v1.
...
BATCH = 2048
FIT_TARGET_FILL = 0.965
EXECUTION_LIMIT_FILL = 0.97
...
def score_oracle(cp, np, matrices, projection, pairs, alpha, cell, prior):
    ...
    fit_target_bits = FIT_TARGET_FILL * payload_bits
    execution_limit_bits = EXECUTION_LIMIT_FILL * payload_bits
    rate_ledger = {
        "ideal_kl_bits_by_expert": kls,
        "fit_target_fraction": FIT_TARGET_FILL,
        "fit_target_bits_by_expert": fit_target_bits,
        "execution_limit_fraction": EXECUTION_LIMIT_FILL,
        "execution_limit_bits_by_expert": execution_limit_bits,
        "physical_reservoir_bits_by_expert": payload_bits,
        "execution_limit_margin_bits_by_expert": [execution_limit_bits - value for value in kls],
        "physical_reservoir_margin_bits_by_expert": [payload_bits - value for value in kls],
    }
    if any(value > payload_bits + 2e-7 for value in kls):
        return {"status": "HARD_KILL_IDEAL_KL_EXCEEDS_PHYSICAL_RESERVOIR", **rate_ledger}
    if any(value > execution_limit_bits + 2e-7 for value in kls):
        return {"status": "HARD_KILL_IDEAL_KL_EXCEEDS_PREREGISTERED_0P97_EXECUTION_LIMIT",
                **rate_ledger}
...
def run_controls(cp, np, matrices, projection, pairs, cell):
    rows = []
    target = FIT_TARGET_FILL * cell["payload_bytes_per_expert"] * 8 / BLOCKS_PER_EXPERT
...
def main():
    ...
    result = {
        "schema": "qwen_stochastic_binary_channel_ptq_stage0_result_v1",
        ...
        "claim_boundary": "Adapted optimistic source-leaking stage-0 topology bound only: v1 budget fractions were chosen after v0 results on this same panel. This is not independent held-out evidence, a serialized channel simulator, an operational Gaussian/TCQ result, a Shannon-limit result, a compressed artifact, or a validation result.",
