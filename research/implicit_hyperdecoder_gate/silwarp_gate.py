...
def moment_matched_control(
    ...
    if np.float16(control_moments["serialized_mean_fp16"]).tobytes() != np.float16(
        moments["serialized_mean_fp16"]
    ).tobytes():
        raise AssertionError("control serialized mean mismatch")
    if np.float16(control_moments["serialized_rms_fp16"]).tobytes() != np.float16(
        moments["serialized_rms_fp16"]
    ).tobytes():
        raise AssertionError("control serialized RMS mismatch")
...
def source_free_preflight() -> dict[str, Any]:
    ...
    return {
        "schema": "silwarp_source_free_preflight_v1",
...
def gpu_source_free_preflight() -> dict[str, Any]:
    ...
    cp.cuda.Stream.null.synchronize()
    receipt = {
        "schema": "silwarp_gpu_source_free_preflight_v1",
        "protocol_sha256": sw.protocol_sha256(),
        "source_lock_sha256": sw.source_lock_sha256(),
        "runner_sha256": sw.sha256_file(Path(__file__).resolve()),
        "common_sha256": sw.sha256_file(Path(sw.__file__).resolve()),
...
def save_training_checkpoint(
    ...
    metadata: dict[str, Any] = {
        "schema": "silwarp_training_checkpoint_v1",
        "update": update,
...
def restore_latest_checkpoint(
    ...
    for chain_update in expected_updates:
        ...
        if chain_metadata.get("schema") != "silwarp_training_checkpoint_v1":
...
def run_gate(
    ...
    result = {
        "schema": "silwarp_auxiliary_result_v1",
        "decision": decision,
