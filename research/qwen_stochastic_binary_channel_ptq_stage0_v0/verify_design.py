#!/usr/bin/env python3
"""Standard-library, source-only verifier for the frozen QSB-PTQ-v0 package."""
...
def verify(package: Path):
    ...
    c.require(manifest.get("schema") == "qwen_stochastic_binary_channel_ptq_stage0_manifest_v0",
              "manifest schema")
    ...
    c.require(design.get("schema") == "qwen_stochastic_binary_channel_ptq_stage0_design_v0",
              "design schema")
    c.require(design.get("status") == "FROZEN_SOURCE_ONLY_NO_PAYLOAD_OR_EXECUTION_AUTHORITY",
              "design source-only status")
    c.require(bindings.get("schema") == "qwen_stochastic_binary_channel_ptq_panel_bindings_v0",
              "panel binding schema")
    c.require(receipt.get("schema") == "qwen_stochastic_binary_channel_ptq_stage0_source_receipt_v0",
              "receipt schema")
    ...
    c.require("97%" in protocol["rate_gate"] and "before" in protocol["early_control_gate"] and
    ...
    c.require("HARD_KILL_IDEAL_KL_EXCEEDS_FROZEN_RESERVOIR" in runner_text and
              "HARD_KILL_FAVOURABLE_ORACLE_UCB_BELOW_EXACT_REQUIREMENT" in runner_text,
              "runner hard-kill gates")
...
def main(argv=None):
    ...
    try:
        ...
        print(json.dumps({"schema": "qwen_stochastic_binary_channel_ptq_stage0_verify_v0",
