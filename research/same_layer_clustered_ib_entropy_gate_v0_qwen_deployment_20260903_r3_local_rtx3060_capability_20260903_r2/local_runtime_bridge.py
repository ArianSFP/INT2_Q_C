...
AUTHORIZATION = "EXECUTE_CBIB1_R3_LOCAL3060_BRIDGE_ONCE_R2_09F4C6D1"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.authorization != AUTHORIZATION:
        print(json.dumps({"schema": "cbib1-r3-local3060-bridge-hold-v0-r2",
                          "status": "HOLD_NO_PATH_RUNTIME_OR_PAYLOAD_ACCESS"},
                         sort_keys=True, separators=(",", ":")))
        return 2
    real_file(ATTEMPT_CLAIM)
    claim = json.loads(ATTEMPT_CLAIM.read_bytes())
    need(claim.get("schema") == "cbib1-r3-local3060-authority-attempt-claim-v0-r2" and
