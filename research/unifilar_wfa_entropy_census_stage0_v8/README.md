...
The only measured object is one complete `UWFCV8` byte string:
...
The total primary ladder first forms connected components of the exact
stream-owner bipartite graph. Any shared stream joins all of its owners, so the
resulting outer units are disjoint and duplicating owner labels cannot create
independent observations. At least two estimable components are required.
Every component fold searches all 150 cells on other components, selects using
the one shared integer literal-container grammar (model, directory, 64-byte
frames, 4-KiB regions and frozen rate padding), refits on all development
streams, and scores the untouched component with the same literal measurement.
The real serializer asserts its emitted frame, region, and total bytes equal
that measurement. The outer score is one direct physical delta: authenticated
current payload/framing with zero UWFA-model bytes versus the candidate layout
containing the selected serialized model exactly once. The shared grammar moves
the following directory to the correct 4-KiB boundary, so no equal-model
cancellation or raw model-byte subtraction can undercharge the model slot.
Promotion requires pooled literal saving of at least
0.15288996696291447 bpw and strictly positive saving in every component. No iid
Student-t claim is made; leave-one-component-out values are diagnostic only.
Legal panels with fewer than two components or no development source return a
typed HOLD before fitting. The stricter layer-and-expert-coordinate-disjoint
split remains explicitly nonpromoting.
...
sealed `nvidia-smi`/NVML identity receipt. Validators independently enforce the
canonical nonempty UUID and PCI grammars and require exact CUDA/NVML/
`nvidia-smi` UUID, PCI, and device-name equality.
...
The validator also recomputes exact count/length symbol updates, kernel,
pack/subset/to-host call counts, and every H2D/D2H category from the frozen
fixture and selected model; category conservation alone is insufficient.
...
eight controls), three disjoint dependence-component outer folds plus final
fit/score, at 50% of measured throughput. The source-free 128-expert fixture proves portability, not full
payload runtime.
...
rebinds both final and marker names to their retained fds, removes only the
same-inode anchor, requires marker link count one, and rereads/rehashes marker
bytes through the held fd after linking, after parent fsync, and before return.
It also rehashes the held directory again. Independent consumers require this parent marker, its expected
source root, parent/directory/marker inode commitments, exact sorted member
table, domain-separated directory root, and matching `COMPLETE.json`. The
bounded verified-bundle API retains marker, directory, and every member fd for
the complete consumer lifetime and exposes only bytes read and revalidated
through those held fds under frozen per-member and aggregate caps. A moved
...
- an externally pinned v8 source audit;
