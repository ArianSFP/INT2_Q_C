#!/usr/bin/env python3
"""Authenticated-snapshot producer core for universal UWFA-SC v8.
...
PRODUCER_ABI = "uwfa-sc-v8-authenticated-snapshot-producer-abi-1"
...
def _validate_environment_identity(protocol: Any, environment: Any, independent: Mapping[str, Any]) -> dict[str, Any]:
    ...
    device_uuid = protocol.canonical_gpu_uuid(row["device_uuid"], "CUDA device UUID")
    pci_bus_id = protocol.canonical_pci_bus_id(row["pci_bus_id"], "CUDA PCI bus id")
    ...
    if independent_row["schema"] != "uwfa-sc-v8-independent-gpu-identity" or independent_row["status"] != "PASS_INDEPENDENT_GPU_IDENTITY":
    ...
    if (
        protocol.canonical_gpu_uuid(independent_row["device_uuid"], "independent GPU UUID") != device_uuid
        or protocol.canonical_pci_bus_id(independent_row["pci_bus_id"], "independent GPU PCI") != pci_bus_id
...
def _expected_all150_workload(common: Any, fixture_symbols: int, streams: int) -> dict[str, int]:
    candidates = common.candidate_bank()
    state_sum = sum(int(candidate.states) for candidate in candidates)
    count_calls = 2 * len(candidates)
    length_calls = 3 * len(candidates)
    kernel_count = count_calls + length_calls
    h2d_payload = 4 * fixture_symbols
    h2d_root = 16 * streams
    h2d_launch = 16 * streams * kernel_count
    h2d_model = 3 * 2 * common.CONTEXTS * state_sum
    h2d_scalars = 16 * kernel_count
    count_d2h = 2 * state_sum * common.CONTEXTS * 2 * 8
    length_d2h = length_calls * streams * 8
    expected = {
        "h2d_payload_bytes": h2d_payload,
        "h2d_root_descriptor_bytes": h2d_root,
        "h2d_subset_descriptor_bytes": 0,
        "h2d_launch_descriptor_bytes": h2d_launch,
        "h2d_model_table_bytes": h2d_model,
        "h2d_kernel_scalar_bytes": h2d_scalars,
        "d2h_bytes": count_d2h + length_d2h,
        "d2d_descriptor_bytes": 0,
        "device_output_allocation_bytes": count_d2h + length_d2h,
        "kernel_count": kernel_count,
        "count_kernel_count": count_calls,
        "length_kernel_count": length_calls,
        "count_cell_symbol_updates": count_calls * fixture_symbols,
        "length_cell_symbol_updates": length_calls * fixture_symbols,
        "pack_calls": 1,
        "subset_calls": 0,
        "to_host_calls": kernel_count,
        "resource_preflight_calls": 1,
    }
    expected["h2d_bytes"] = sum(expected[name] for name in (
        "h2d_payload_bytes", "h2d_root_descriptor_bytes", "h2d_subset_descriptor_bytes",
        "h2d_launch_descriptor_bytes", "h2d_model_table_bytes", "h2d_kernel_scalar_bytes",
    ))
    return expected
...
def _validate_all150_receipt(common: Any, protocol: Any, value: Any, bindings: BoundEvidence, independent: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if row["schema"] != "uwfa-sc-v8-all150-source-free-preflight" or row["status"] != "PASS_ALL_150_CPU_CUPY_EXACT_REPEATED":
    ...
    expected_workload = _expected_all150_workload(common, fixture_symbols, 4)
    observed_workload = environment["statistics"]
    for name, expected in expected_workload.items():
        if int(observed_workload[name]) != expected:
            raise ValueError(f"all150 telemetry/workload mismatch: {name}")
...
def _validate_representative_receipt(common: Any, protocol: Any, value: Any, bindings: BoundEvidence, independent: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if row["schema"] != "uwfa-sc-v8-representative-source-free-preflight" or row["status"] != "PASS_REPRESENTATIVE_SOURCE_FREE_OUTER_FOLD":
    ...
    expected_development = list(range(4, 12)) + [13, 14]
    if outer["train_streams"] != list(range(6, 12)) + [13, 14] or outer["validation_streams"] != [4, 5] or outer["test_streams"] != [0, 1, 2, 3, 12]:
    ...
    test_lengths, test_sha = _u64_list_sha256(protocol, outer["test_logical_lengths"], count=5, label="representative test length")
    ...
    projected_updates = 13 * (3 * one_fold_updates + 2 * full_symbols)
    ...
    if (
        ...
        or runtime["projection_formula"] != "13 pipelines * (3 disjoint dependence-component outer folds + final fit/score), at 50% measured throughput"
    ...
    selected_states = int(expected_winner["states"])
    count_calls = 152
    length_calls = 152
    count_stream_uses = 150 * len(outer["train_streams"]) + len(expected_development) + 15
    length_stream_uses = 150 * len(outer["validation_streams"]) + len(outer["test_streams"]) + 15
    count_updates = 150 * train_symbols + development_symbols + full_symbols
    length_updates = 150 * validation_symbols + test_symbols + full_symbols
    candidate_state_sum = sum(int(candidate.states) for candidate in common.candidate_bank())
    expected_delta = {
        "h2d_payload_bytes": 4 * full_symbols,
        ...
        "h2d_model_table_bytes": 2 * common.CONTEXTS * (candidate_state_sum + 2 * selected_states),
        "h2d_kernel_scalar_bytes": 16 * (count_calls + length_calls),
        "d2h_bytes": (
            common.CONTEXTS * 2 * 8 * (candidate_state_sum + 2 * selected_states)
            + 8 * (150 * len(outer["validation_streams"]) + len(outer["test_streams"]) + 15)
        ),
        "d2d_descriptor_bytes": 0,
        "kernel_count": count_calls + length_calls,
        "count_kernel_count": count_calls,
        "length_kernel_count": length_calls,
        "count_cell_symbol_updates": count_updates,
        "length_cell_symbol_updates": length_updates,
        "pack_calls": 1,
    ...
    for name, expected in expected_delta.items():
        if clean_delta[name] != expected:
            raise ValueError(f"representative telemetry/workload mismatch: {name}")
    if count_updates + length_updates != measured_updates:
        raise ValueError("representative phase symbol updates do not equal measured workload")
...
def _literal_panel_layout_bits(
    protocol: Any,
    container_codec: Any,
    ...
    *,
    ...
    """Charge one exact baseline/candidate literal layout without payload allocation.

    Replacement streams use the supplied exact arithmetic lengths. Every other
    stream retains authenticated current-format payload geometry. A baseline
    passes zero UWFA model bytes; a candidate passes the exact serialized model
    length. Thus the shared grammar itself charges model placement, directory
    displacement, frames, regions and padding in one physical byte delta.
    """
    streams = panel.get("streams")
    if not isinstance(streams, list) or not streams:
        raise ValueError("literal validation panel streams")
    indices = [protocol.exact_int(value, "literal validation index", 0, len(streams) - 1) for value in validation_indices]
    if indices != sorted(set(indices)) or len(indices) != len(logical_lengths):
        raise ValueError("literal validation index/length geometry")
    replacements = {
        index: protocol.exact_int(logical, "literal validation logical bits", 1, protocol.MAX_LOGICAL_BITS)
        for index, logical in zip(indices, logical_lengths, strict=True)
    }
    model_bytes = protocol.exact_int(
        model_packet_bytes, "literal layout model bytes", 0,
        container_codec.MAX_MODEL_BYTES,
    )
    experts = protocol.validate_experts(panel.get("experts"))
    grouped: dict[bytes, list[dict[str, int]]] = {}
    for index, row in enumerate(streams):
        owner_set = bytes(row["owner_set"])
        protocol.owner_ordinals(owner_set, experts)
        if index in replacements:
            logical = replacements[index]
            payload_bytes = (logical + 7) // 8
        else:
            logical = protocol.exact_int(
                row["baseline_logical_bits"], "literal baseline logical bits", 1,
                protocol.MAX_LOGICAL_BITS,
            )
            payload_bytes = protocol.exact_int(
                row["baseline_payload_bytes"], "literal baseline payload bytes", 1,
                container_codec.MAX_FRAME_BYTES,
            )
        contributions = row.get("owner_contributions")
        if not isinstance(contributions, (tuple, list)) or not contributions:
            raise ValueError("literal validation owner contributions")
        grouped.setdefault(owner_set, []).append({
    ...
    measured = container_codec.measure_literal_container_layout(
        semantic_packet_bytes=len(bytes(panel["semantic_packet"])),
        immutable_state_bytes=len(bytes(panel["immutable_state"])),
        model_packet_bytes=model_bytes,
        weights=protocol.exact_int(panel["weights"], "literal validation weights", 1, protocol.MAX_WEIGHTS),
        experts=experts,
        regions=[{"owner_set": owner_set, "frames": grouped[owner_set]} for owner_set in owner_sets],
        minimum_rate_numerator=container_codec.RATE_MIN_NUMERATOR,
        minimum_rate_denominator=container_codec.RATE_MIN_DENOMINATOR,
    )
    return 8 * int(measured["total_bytes"])


def literal_validation_score(
    common: Any,
    protocol: Any,
    container_codec: Any,
    panel: Mapping[str, Any],
    ...
    """Inner/candidate score with the selected serialized model exactly once."""
    model_packet = common.serialize_model(candidate, frequencies)
    return _literal_panel_layout_bits(
        protocol, container_codec, panel, validation_indices, logical_lengths,
        model_packet_bytes=len(model_packet),
    )


def literal_current_baseline_score(
    protocol: Any,
    container_codec: Any,
    panel: Mapping[str, Any],
) -> int:
    """Authenticated current payload/framing baseline with no UWFA model."""
    return _literal_panel_layout_bits(
        protocol, container_codec, panel, [], [], model_packet_bytes=0,
    )
...
def representative_outer_fold_benchmark(
    ...
    semantic_packet = semantic_codec.build_semantic_packet(shapes, b"representative-outer-fold-v8")
    ...
    for ordinal, (owners, role, length) in enumerate(zip(owner_layout, roles, public_lengths, strict=True)):
        ...
        rows.append({
            ...
            "baseline_payload_bytes": (length + 7) // 8,
        })
    ...
    held_component = {0, 1}
    development = [index for index, row in enumerate(rows) if held_component.isdisjoint(row["owners"])]
    test = [index for index, row in enumerate(rows) if not held_component.isdisjoint(row["owners"])]
    ...
    for candidate in common.candidate_bank():
        ...
        charged = literal_validation_score(
            common, protocol, container_codec, selection_panel,
            validation, lengths, candidate, frequencies,
        )
    ...
    projected_updates = pipelines * (3 * one_fold_updates + 2 * full_symbols)
...
def _split_digest(protocol: Any, identity: tuple[int, int], row: Mapping[str, Any]) -> bytes:
    digest = protocol.length_prefixed_digest(
        ...
        domain=b"UWFA-SC-V8-NESTED-SPLIT-2026-09-02\x00",
    )
...
def _dependence_components(protocol: Any, panel: Mapping[str, Any]) -> list[tuple[int, ...]]:
    """Connected components of the exact stream-owner bipartite graph."""
    experts = protocol.validate_experts(panel.get("experts"))
    ...
    for row in panel["streams"]:
        owners = [
            protocol.exact_int(value, "dependence owner", 0, experts - 1)
            for value in row["owner_identity_indices"]
        ]
        if owners != sorted(set(owners)) or not owners:
            raise ValueError("dependence owner geometry")
...
def _component_fold_plan(common: Any, protocol: Any, panel: Mapping[str, Any]) -> list[dict[str, Any]]:
    streams = panel["streams"]
    identities = panel["semantic_identities"]
    plans = []
    for component_ordinal, members in enumerate(_dependence_components(protocol, panel)):
        member_set = set(members)
        test_indices = [
            index for index, row in enumerate(streams)
            if member_set.intersection(row["owner_identity_indices"])
        ]
        development_indices = [
            index for index, row in enumerate(streams)
            if not member_set.intersection(row["owner_identity_indices"])
        ]
        # Connectivity proves no stream can straddle two components.
        if any(not set(streams[index]["owner_identity_indices"]).issubset(member_set) for index in test_indices):
            raise ValueError("dependence component does not close shared stream")
        estimable = bool(test_indices) and len(development_indices) >= 2
        validation: list[int] = []
        train: list[int] = []
        if estimable:
            component_key = [
                value for member in members for value in identities[member]
            ]
            ranked = sorted(
                development_indices,
                key=lambda index: (
                    protocol.length_prefixed_digest(
                        component_key + [int(streams[index]["stream_ordinal"]), bytes(streams[index]["owner_set"])],
            ...
            validation_count = min(
                max(1, len(ranked) // common.INNER_VALIDATION_MODULUS),
                len(ranked) - 1,
            )
            validation = sorted(ranked[:validation_count])
            train = sorted(ranked[validation_count:])
        plans.append({
            ...
            "train_indices": train,
            "estimable": estimable,
            "reason": None if estimable else "requires a nonempty closed component and at least two streams in other components",
        })
    return plans
...
def projected_updates(common: Any, protocol: Any, panel: Mapping[str, Any]) -> dict[str, Any]:
    ...
    folds = []
    streams = panel["streams"]
    exact_plans = _component_fold_plan(common, protocol, panel)
    ...
    for plan in exact_plans:
        ...
        if not plan["estimable"]:
            folds.append({"component_ordinal": component_ordinal, "identity_indices": plan["identity_indices"], "estimable": False, "cell_symbol_updates": 0})
            continue
        ...
        total += updates
        folds.append({"component_ordinal": component_ordinal, "identity_indices": plan["identity_indices"], "estimable": True, "cell_symbol_updates": updates})
    ...
    return {
        ...
        "primary_exact_identity_estimable": len(folds) >= 2 and all(bool(row["estimable"]) for row in folds),
        "disjoint_dependence_component_count": len(folds),
        ...
        for context, positions in groups.items():
            ranked_sources = sorted(
                ...
                key=lambda position: protocol.length_prefixed_digest(
                    ...
                    domain=b"UWFA-SC-V8-WITHIN-CONTEXT-PERMUTATION\x00",
                ),
            )
    ...
    for row in panel["streams"]:
        ...
        ranked = sorted(
            ...
            key=lambda item: protocol.length_prefixed_digest(
                ...
                domain=b"UWFA-SC-V8-MULTISCALE-CHUNK-SHUFFLE\x00",
            ),
        )
...
def survivor_shuffle_diagnostics(common: Any, protocol: Any, container_codec: Any, backend: Any, panel: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for name, variant in variants:
        ...
        scientific = nested_holdout(common, protocol, container_codec, backend, cache, variant)
        rows.append({
            "variant": name,
...
def nested_holdout(
    ...
    *,
    ...
    streams = panel["streams"]
    identities = panel["semantic_identities"]
    if policy == "exact_identity" and not diagnostic_only:
        plans = _component_fold_plan(common, protocol, panel)
        if len(plans) < 2:
            return {
                "kind": "disjoint_stream_owner_dependence_component_holdout",
                "primary_policy": "disjoint_stream_owner_dependence_components",
                "status": "HOLD_NONESTIMABLE_DEPENDENCE_COMPONENTS",
                "folds": [],
                "skipped_folds": [],
                "dependence_components": [plan["identity_indices"] for plan in plans],
                "independent_component_count": len(plans),
                "estimable": False,
    ...
    skipped = [
        {
            "outer_unit": (
                {"component_ordinal": int(plan["component_ordinal"]), "identity_indices": list(plan["identity_indices"])}
                if "component_ordinal" in plan
                else {"identity_index": int(plan["identity_index"]), "identity": list(plan["identity"])}
            ),
            "reason": plan["reason"],
        }
    ...
    for plan in valid_plans:
        component_mode = "component_ordinal" in plan
        identity_indices = (
            [int(value) for value in plan["identity_indices"]]
            if component_mode else [int(plan["identity_index"])]
        )
        test_indices = plan["test_indices"]
        development_indices = plan["development_indices"]
        ...
        for candidate in common.candidate_bank():
            ...
            selections.append((
                literal_validation_score(
                    common, protocol, container_codec, panel,
                    validation_indices, lengths, candidate, frequencies,
                ),
                candidate.selector_ordinal,
                candidate,
            ))
        ...
        literal_baseline_bits = literal_current_baseline_score(
            protocol, container_codec, panel,
        )
        literal_model_only_bits = literal_validation_score(
            common, protocol, container_codec, panel, [], [], selected, frequencies,
        )
        literal_candidate_bits = literal_validation_score(
            common, protocol, container_codec, panel,
            test_indices, test_lengths, selected, frequencies,
        )
        ...
        # One shared literal-container delta is the complete physical charge.
        # Baseline has authenticated current payload geometry and no UWFA
        # model; candidate contains the selected model exactly once. Never
        # cancel equal model layouts and never subtract raw model bytes.
        literal_model_aligned_increment_bits = literal_model_only_bits - literal_baseline_bits
        literal_saved_bits = literal_baseline_bits - literal_candidate_bits
        saving = literal_saved_bits / allocated_weights
        outer = {
            "outer_dependence_component_ordinal": (
                int(plan["component_ordinal"]) if component_mode else None
            ),
            "outer_identity_indices": identity_indices,
            "outer_identities_from_artifact": [list(identities[index]) for index in identity_indices],
            ...
            "literal_selected_model_aligned_increment_bits": literal_model_aligned_increment_bits,
    ...
    pooled_saved_bits = sum(float(row["literal_test_saving_after_exact_container_delta_bits"]) for row in fold_rows)
    pooled = pooled_saved_bits / allocated_total
    values = [float(row["exact_test_saving_bpw"]) for row in fold_rows]
    mean = statistics.fmean(values)
    # Components are disjoint estimand units, but not assumed iid. Promotion
    # therefore uses a conservative conjunction (pooled target plus every
    # component positive), never a Student-t interval over overlapping folds.
    ...
    if len(fold_rows) > 1:
        for omitted, row in enumerate(fold_rows):
            ...
            kept_bits = pooled_saved_bits - float(row["literal_test_saving_after_exact_container_delta_bits"])
    ...
    primary = not diagnostic_only
    pass_gate = primary and pooled >= common.STANDALONE_REQUIRED_SAVING_BPW and component_positive
    return {
        "kind": "coordinate-disjoint-nonpromoting-diagnostic" if diagnostic_only else "disjoint_stream_owner_dependence_component_holdout",
        "primary_policy": policy,
        "status": "PASS_NONPROMOTING_COORDINATE_DISJOINT_DIAGNOSTIC" if diagnostic_only else "PASS_DISJOINT_DEPENDENCE_COMPONENT_HOLDOUT",
        "folds": fold_rows,
        "skipped_folds": skipped,
        "estimable": True,
        "pooled_exact_heldout_saving_bpw": pooled,
        "minimum_fold_exact_saving_bpw": min(values),
        "dependence_component_mean_saving_bpw_diagnostic_only": mean,
        "confidence_rule": "no iid confidence interval; promotion requires disjoint owner-stream components, pooled literal saving at target, and every component strictly positive",
        ...
        "passes_heldout_gate": pass_gate,
        "positive_promotion": False,
    }


def coordinate_disjoint_diagnostic(common: Any, protocol: Any, container_codec: Any, backend: Any, cache: Any, panel: Mapping[str, Any]) -> dict[str, Any]:
    return nested_holdout(
        common, protocol, container_codec, backend, cache, panel,
...
def source_phase(
    *,
    ...
    if not runtime_projection["primary_exact_identity_estimable"]:
        return {
            "schema": "uwfa-sc-v8-source-phase-result",
    ...
    scientific = nested_holdout(common, protocol, container_codec, backend, cache, panel)
    ...
    if status == "SOURCE_SURVIVOR_CONTROLS_AUTHORIZED_NOT_YET_OPENED":
        shuffles = survivor_shuffle_diagnostics(common, protocol, container_codec, backend, panel)
        stricter_diagnostic = coordinate_disjoint_diagnostic(common, protocol, container_codec, backend, cache, panel)
    elapsed = time.perf_counter() - started
    result = {
        "schema": "uwfa-sc-v8-source-phase-result",
        "status": status,
...
def controls_phase(
    *,
    ...
    return {
        "schema": "uwfa-sc-v8-control-phase-result",
