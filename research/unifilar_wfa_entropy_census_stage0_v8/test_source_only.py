#!/usr/bin/env python3
"""Hostile source-only UWFA-SC v8 producer tests.
...
common = load("uwfa_v8_test_common", "uwfa_common.py")
protocol = load("uwfa_v8_test_protocol", "protocol.py")
semantic = load("uwfa_v8_test_semantic", "universal_adapter.py")
codec = load("uwfa_v8_test_container", "container_codec.py")
fixture = load("uwfa_v8_test_fixture", "fixture_portability.py")
strata = load("uwfa_v8_test_strata", "strata_sc_adapter.py")
stage = load("uwfa_v8_test_stage", "stage0_census.py")
cupy_backend = load("uwfa_v8_test_cupy", "cupy_backend.py")
dispatcher = load("uwfa_v8_test_dispatcher", "dispatcher_contract.py")
envelope = load("uwfa_v8_test_envelope", "result_envelope.py")
...
def _synthetic_environment(last_pack_resource_plan: dict[str, Any], workload: dict[str, int] | None = None) -> dict[str, Any]:
    ...
    if workload is not None:
        statistics.update({name: int(value) for name, value in workload.items()})
    sample = {
...
def _valid_source_preflight_payload() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], str]:
    ...
    all150 = {
        "schema": "uwfa-sc-v8-all150-source-free-preflight",
        "source_snapshot_root_sha256": source_root,
        ...
        "environment": _synthetic_environment(
            all150_resource,
            stage._expected_all150_workload(common, fixture_symbols, 4),
        ),
    ...
    train_streams = list(range(6, 12)) + [13, 14]
    validation_streams = [4, 5]
    test_streams = [0, 1, 2, 3, 12]
    development_streams = list(range(4, 12)) + [13, 14]
    ...
    projected_updates = 13 * (3 * one_fold_updates + 2 * representative_symbols)
    ...
    test_lengths = [300 + index for index in range(5)]
    ...
    winner_states = int(common.candidate_bank()[0].states)
    candidate_state_sum = sum(int(candidate.states) for candidate in common.candidate_bank())
    count_stream_uses = 150 * len(train_streams) + len(development_streams) + 15
    length_stream_uses = 150 * len(validation_streams) + len(test_streams) + 15
    phase_delta = {
        "h2d_payload_bytes": 4 * representative_symbols,
        "h2d_root_descriptor_bytes": 16 * 15,
        "h2d_subset_descriptor_bytes": 16 * (count_stream_uses + length_stream_uses),
        "h2d_launch_descriptor_bytes": 16 * (count_stream_uses + length_stream_uses),
        "h2d_model_table_bytes": 2 * common.CONTEXTS * (candidate_state_sum + 2 * winner_states),
        "h2d_kernel_scalar_bytes": 16 * 304,
        "d2d_descriptor_bytes": 0,
        "kernel_count": 304,
        "count_kernel_count": 152,
        "length_kernel_count": 152,
        "count_cell_symbol_updates": 150 * train_symbols + development_symbols + representative_symbols,
        "length_cell_symbol_updates": 150 * validation_symbols + test_symbols + representative_symbols,
        "pack_calls": 1,
        "subset_calls": 304,
        "to_host_calls": 304,
    }
    phase_delta["h2d_bytes"] = sum(phase_delta[name] for name in (
    ...
    phase_delta["d2h_bytes"] = (
        common.CONTEXTS * 2 * 8 * (candidate_state_sum + 2 * winner_states)
        + 8 * (150 * len(validation_streams) + len(test_streams) + 15)
    )
    phase_delta["device_output_allocation_bytes"] = phase_delta["d2h_bytes"]
    representative = {
        "schema": "uwfa-sc-v8-representative-source-free-preflight",
        "source_snapshot_root_sha256": source_root,
        ...
        "runtime_projection": {
            ...
            "projection_formula": "13 pipelines * (3 disjoint dependence-component outer folds + final fit/score), at 50% measured throughput",
...
def build_fixture(experts: int = 2, hidden: int = 3, intermediate: int = 5, *, shared: bool = True) -> tuple[bytes, dict[str, Any], dict[str, Any]]:
    ...
    raw, metrics = codec.build_container(
        ...
        immutable_state=b"source-only-v8-fixture",
...
class LiteralContainerTests(unittest.TestCase):
    ...
    def test_shared_measure_only_grammar_equals_actual_serialized_bytes(self) -> None:
        measured = codec.measure_literal_container_layout(
            semantic_packet_bytes=len(self.source["semantic_packet"]),
            immutable_state_bytes=len(b"source-only-v8-fixture"),
            model_packet_bytes=len(self.source["model_packet"]),
            weights=self.source["weights"],
            experts=self.source["experts"],
            regions=[
                {
        ...
        self.assertEqual(measured["total_bytes"], len(self.raw))
        self.assertEqual(measured["region_bytes"], [row["bytes"] for row in self.parsed["regions"]])

    def test_literal_alignment_can_reverse_raw_payload_candidate_order(self) -> None:
        owner = protocol.owner_set_from_ordinals(1, [0])

        def measured(payloads: list[int]) -> int:
            row = codec.measure_literal_container_layout(
                semantic_packet_bytes=128,
                immutable_state_bytes=0,
                model_packet_bytes=64,
                weights=3,
                experts=1,
                regions=[{
                    "owner_set": owner,
                    "frames": [
                        {"logical_bits": 8 * payload, "payload_bytes": payload, "contribution_count": 1}
        ...
        raw_proxy_favored = [1, 1345, 1409]
        literal_favored = [1, 1344, 1472]
        self.assertLess(sum(raw_proxy_favored), sum(literal_favored))
        self.assertGreater(measured(raw_proxy_favored), measured(literal_favored))
        self.assertEqual(measured(raw_proxy_favored) - measured(literal_favored), codec.PAGE_BYTES)

    def test_outer_model_charge_uses_aligned_literal_increment_not_raw_bytes(self) -> None:
        owner = protocol.owner_set_from_ordinals(1, [0])
        candidate = common.Candidate("suffix", 2, 32)
        model_packet = common.serialize_model(
            candidate, [32768] * common.model_frequency_count(candidate)
        )
        self.assertEqual(len(model_packet), 1602)

        def measured(*, model_bytes: int, payload_bytes: int) -> int:
            return int(codec.measure_literal_container_layout(
                semantic_packet_bytes=128,
                immutable_state_bytes=0,
                model_packet_bytes=model_bytes,
                weights=1,
                experts=1,
                regions=[{
                    "owner_set": owner,
                    "frames": [{
                        "logical_bits": 8 * payload_bytes,
        ...
        baseline = measured(model_bytes=0, payload_bytes=7000)
        model_only = measured(model_bytes=len(model_packet), payload_bytes=7000)
        candidate_total = measured(model_bytes=len(model_packet), payload_bytes=5000)
        self.assertEqual(model_only - baseline, codec.PAGE_BYTES)
        self.assertNotEqual(model_only - baseline, len(model_packet))
        # The retired proxy would report a 398-byte win. Exact container
        # placement reports a 4096-byte loss because the payload reduction
        # remains inside the same region page while the model moves the
        # directory/shared boundary by one full page.
        self.assertGreater((7000 - 5000) - len(model_packet), 0)
        self.assertEqual(baseline - candidate_total, -codec.PAGE_BYTES)

        boundary_increments = []
        for states in (4, 8):
            item = common.Candidate("suffix", states, 32)
            packet = common.serialize_model(item, [32768] * common.model_frequency_count(item))
            boundary_increments.append(measured(model_bytes=len(packet), payload_bytes=7000) - baseline)
        self.assertEqual(boundary_increments, [codec.PAGE_BYTES, 2 * codec.PAGE_BYTES])
...
class PortabilityTests(unittest.TestCase):
    ...
    def test_e250_unequal_shape_shared_tail_descriptor_portability(self) -> None:
        ...
        raw, _diagnostic = codec.build_container(
            ...
            immutable_state=b"e250-unequal-v8", regions=source["regions"],
...
def scientific_panel() -> dict[str, Any]:
    ...
    for expert in range(6):
        for role_index, role in enumerate(("gate", "up", "down")):
            ...
            baseline_payload = bytes((length + 7) // 8)
            ...
            rows.append({
                ...
                "source_digest": common.selected_decision_triplet_sha256(bits, levels, base_bytes),
                "profile_q": 0,
                "decoder_scale": 1.0,
                "logn": 1,
            })
    shapes = tuple(semantic.ExpertShape(index, 1, 1) for index in range(6))
    return {
        "streams": rows,
        "weights": 18,
        "experts": 6,
        ...
        "immutable_state": b"",
        "semantic_packet": semantic.build_semantic_packet(shapes, b"scientific-panel-v8"),
...
class ScientificPipelineTests(unittest.TestCase):
    def test_fold_measurement_equals_actual_serialized_candidate_container(self) -> None:
        panel = scientific_panel()
        candidate = common.Candidate("suffix", 2, 32)
        frequencies = [32768] * common.model_frequency_count(candidate)
        replacement_logical = int(panel["streams"][0]["baseline_logical_bits"]) + 64
        measured_bits = stage.literal_validation_score(
            common, protocol, codec, panel, [0], [replacement_logical], candidate, frequencies,
        )
        specs = []
        for index, row in enumerate(panel["streams"]):
            logical = replacement_logical if index == 0 else int(row["baseline_logical_bits"])
            payload = bytes((logical + 7) // 8)
            contributions = tuple(
                codec.OwnerContribution(
                    int(item["expert"]), str(item["role"]),
                    int(item["source_offset"]), int(item["weight_count"]),
                )
                for item in row["owner_contributions"]
            )
            specs.append((codec.StreamSpec(
                ordinal=int(row["stream_ordinal"]), symbols=int(row["symbols"]),
                logical_bits=logical, payload=payload,
                source_digest=str(row["source_digest"]), profile_q=int(row["profile_q"]),
                decoder_scale=float(row["decoder_scale"]), role=str(row["role"]),
                group_rows=int(row["shape_rows"]), group_cols=int(row["shape_cols"]),
                owner_contributions=contributions,
            ), bytes(row["owner_set"])))
        actual, _metrics = codec.build_container(
            common, semantic,
            ...
            regions=stage._regions_from_specs(codec, specs, int(panel["experts"])),
            weights=int(panel["weights"]), experts=int(panel["experts"]),
            baseline_object_bytes=int(panel["artifact"]["raw_bytes"]),
            audited_relative_mse=0.025,
            baseline_artifact_sha256=str(panel["artifact"]["raw_sha256"]),
            reconstruction_sha256=str(panel["reconstruction"]["full_reconstruction_f64_sha256"]),
            audit_binding_sha256="33" * 32, binding_hashes=bindings(),
        )
        self.assertEqual(measured_bits, 8 * len(actual))
    ...
    def test_nested_150_dynamic_confidence_and_exact_weights(self) -> None:
        ...
        result = stage.nested_holdout(common, protocol, codec, backend, cache, panel)
        self.assertEqual(len(result["folds"]), 6)
        self.assertEqual(result["status"], "PASS_DISJOINT_DEPENDENCE_COMPONENT_HOLDOUT")
        self.assertEqual(result["independent_component_count"], 6)
        self.assertNotIn("whole_expert_t95_lower_bpw", result)
        self.assertIn("no iid confidence interval", result["confidence_rule"])
        ...
        strict = stage.coordinate_disjoint_diagnostic(common, protocol, codec, backend, cache, panel)
    ...
    def test_all_owner_shared_stream_collapses_to_one_component_and_holds_without_fit(self) -> None:
        panel = scientific_panel()
        owner = protocol.owner_set_from_ordinals(6, list(range(6)))
        row = panel["streams"][0]
        row["owner_set"] = owner
        row["owner_set_hex"] = owner.hex()
        row["owner_identity_indices"] = list(range(6))
        row["owner_expert_ordinals"] = list(range(6))
        row["owner_weight_contributions"] = {index: 1 for index in range(6)}
        row["owner_contributions"] = tuple(
            {"expert": index, "role": "gate", "source_offset": 0, "weight_count": 1}
            for index in range(6)
        )
        row["weight_charge"] = 6
        panel["weights"] = 23
        backend = mock.Mock()
        result = stage.nested_holdout(common, protocol, codec, backend, object(), panel)
        self.assertEqual(result["status"], "HOLD_NONESTIMABLE_DEPENDENCE_COMPONENTS")
        self.assertEqual(result["independent_component_count"], 1)
        self.assertFalse(result["estimable"])
        self.assertFalse(result["passes_heldout_gate"])
    ...
    def test_legal_single_expert_panel_fails_safe_before_any_fit(self) -> None:
        ...
        backend = mock.Mock()
        panel["semantic_packet"] = semantic.build_semantic_packet((semantic.ExpertShape(0, 1, 1),), b"scientific-single-v8")
        result = stage.nested_holdout(common, protocol, codec, backend, object(), panel)
        self.assertEqual(result["status"], "HOLD_NONESTIMABLE_DEPENDENCE_COMPONENTS")
        self.assertFalse(result["estimable"])
        backend.fit_counts.assert_not_called()
    ...
    def test_preflight_rejects_resealed_telemetry_not_bound_to_measured_workload(self) -> None:
        preflight, evidence = valid_source_preflight()
        forged_all150 = copy.deepcopy(dict(preflight.all150))
        stats = forged_all150["environment"]["statistics"]
        stats["count_cell_symbol_updates"] = 0
        stats["length_cell_symbol_updates"] = 0
        forged, forged_evidence = reseal_source_preflight(
            stage.SourcePreflightEvidence(
                forged_all150,
                copy.deepcopy(dict(preflight.representative)),
                copy.deepcopy(dict(preflight.independent_gpu_identity)),
                preflight.receipt_sha256,
            ),
            evidence,
        )
        with self.assertRaisesRegex(ValueError, "all150 telemetry/workload mismatch"):
            stage.validate_source_preflight(common, protocol, forged, forged_evidence)

        forged_rep = copy.deepcopy(dict(preflight.representative))
        forged_rep["measured_phase_statistics_delta"]["count_cell_symbol_updates"] = 0
        forged_rep["measured_phase_statistics_delta"]["length_cell_symbol_updates"] = 0
        forged, forged_evidence = reseal_source_preflight(
            stage.SourcePreflightEvidence(
                copy.deepcopy(dict(preflight.all150)),
                forged_rep,
                copy.deepcopy(dict(preflight.independent_gpu_identity)),
                preflight.receipt_sha256,
            ),
        ...
        with self.assertRaisesRegex(ValueError, "representative telemetry/workload mismatch"):
            stage.validate_source_preflight(common, protocol, forged, forged_evidence)
...
class TelemetryContractTests(unittest.TestCase):
    ...
    def test_uuid_and_pci_canonicalization_is_exact_and_fail_closed(self) -> None:
        ...
        self.assertEqual(
            protocol.canonical_gpu_uuid("GPU-c06e0fe0-9836-2f98-8f10-0514d085f722", "GPU"),
            "GPU-c06e0fe0-9836-2f98-8f10-0514d085f722",
        )
        self.assertEqual(protocol.canonical_pci_bus_id("00000000:16:00.0", "PCI"), "00000000:16:00.0")
        for hostile in ("", "x", "GPU-C06E0FE0-9836-2f98-8f10-0514d085f722", "GPU-00000000-0000-0000-0000-000000000000"):
            with self.subTest(uuid=hostile), self.assertRaises(ValueError):
                protocol.canonical_gpu_uuid(hostile, "GPU")
        for hostile in ("", "x", "0000:16:00.0", "00000000:16:00.8", "00000000:GG:00.0"):
            with self.subTest(pci=hostile), self.assertRaises(ValueError):
                protocol.canonical_pci_bus_id(hostile, "PCI")
...
class SecurityAndSealTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                with envelope.verify_completed_under_parent(
                    ...
                    self.assertEqual(verified.metadata["status"], "PASS_PARENT_MARKER_COMMITTED_BUNDLE_HELD")
                    self.assertEqual(verified.read_member_bytes("RESULT.bin"), b"durable")
                with self.assertRaisesRegex(ValueError, "not active"):
                    verified.read_member_bytes("RESULT.bin")
            finally:
                retained.close()
    ...
    def test_verified_bundle_consumes_held_bytes_after_final_and_marker_path_replacement(self) -> None:
        if os.name != "posix":
            self.skipTest("descriptor-relative publication is a POSIX production contract")
        with tempfile.TemporaryDirectory() as directory:
            parent_path = Path(directory)
            retained = common.RetainedOutputParent.open_path_source_only(parent_path, "b1" * 32)
            final_name = "lifetime-replaced"
            marker_name = common.parent_commit_marker_name(final_name)
            try:
                with common.CompletionLastOutput(retained, final_name, "31" * 16) as transaction:
                    transaction.write_new("RESULT.bin", b"authenticated-original")
                    transaction.complete(list(transaction.members), "ab" * 32)
                with envelope.verify_completed_under_parent(
                    common, retained, final_name, expected_source_manifest_sha256="ab" * 32,
                ) as bundle:
                    # Verification is complete and all fds are held. Replace
                    # both publication paths before consuming any RESULT byte.
                    os.rename(final_name, "lifetime-original-aside", src_dir_fd=retained.fd, dst_dir_fd=retained.fd)
                    os.mkdir(final_name, 0o700, dir_fd=retained.fd)
                    replacement_fd = os.open(
                        final_name, os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW, dir_fd=retained.fd,
                    )
                    try:
                        fd = os.open("RESULT.bin", os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600, dir_fd=replacement_fd)
                        os.write(fd, b"attacker-replacement")
                        os.close(fd)
                    finally:
                        os.close(replacement_fd)
                    os.rename(marker_name, "lifetime-marker-aside", src_dir_fd=retained.fd, dst_dir_fd=retained.fd)
                    fd = os.open(marker_name, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600, dir_fd=retained.fd)
                    os.write(fd, b"forged-marker")
                    os.close(fd)
                    self.assertEqual(bundle.read_member_bytes("RESULT.bin"), b"authenticated-original")
                    self.assertTrue(bundle.metadata["all_member_directory_and_marker_descriptors_retained"])
            finally:
                retained.close()

    def test_verified_bundle_fails_if_held_member_mutates_after_verification(self) -> None:
        if os.name != "posix":
            self.skipTest("descriptor-relative publication is a POSIX production contract")
        with tempfile.TemporaryDirectory() as directory:
            parent_path = Path(directory)
            retained = common.RetainedOutputParent.open_path_source_only(parent_path, "b2" * 32)
            final_name = "lifetime-mutated"
            try:
                with common.CompletionLastOutput(retained, final_name, "32" * 16) as transaction:
                    transaction.write_new("RESULT.bin", b"authenticated-original")
                    transaction.complete(list(transaction.members), "ab" * 32)
                with envelope.verify_completed_under_parent(
                    common, retained, final_name, expected_source_manifest_sha256="ab" * 32,
                    ...
                    fd = os.open(
                        str(parent_path / final_name / "RESULT.bin"), os.O_WRONLY | os.O_TRUNC | os.O_NOFOLLOW,
                    )
                    os.write(fd, b"mutated")
                    os.fsync(fd)
                    os.close(fd)
                    with self.assertRaisesRegex(ValueError, "held member changed before consumption"):
                        bundle.read_member_bytes("RESULT.bin")
            finally:
                retained.close()

    def test_verified_bundle_freezes_caps_before_member_buffering(self) -> None:
        self.assertEqual(envelope.MAX_BUFFERED_MEMBER_BYTES, 64 << 20)
        self.assertEqual(envelope.MAX_BUFFERED_AGGREGATE_BYTES, 256 << 20)
        if os.name != "posix":
            self.skipTest("descriptor-relative publication is a POSIX production contract")
        with tempfile.TemporaryDirectory() as directory:
            parent_path = Path(directory)
            retained = common.RetainedOutputParent.open_path_source_only(parent_path, "b3" * 32)
            try:
                with common.CompletionLastOutput(retained, "capped", "33" * 16) as transaction:
                    transaction.write_new("RESULT.bin", b"R" * 2048)
                    transaction.complete(list(transaction.members), "ab" * 32)
                with mock.patch.object(envelope, "MAX_BUFFERED_MEMBER_BYTES", 1024):
                    with envelope.verify_completed_under_parent(
                        common, retained, "capped", expected_source_manifest_sha256="ab" * 32,
                    ) as bundle:
                        self.assertEqual(bundle.metadata["buffer_caps"]["per_member_bytes"], 1024)
                        with self.assertRaisesRegex(ValueError, "exceeds buffer cap"):
                            bundle.read_member_bytes("RESULT.bin")
            finally:
                retained.close()
    ...
    def test_parent_marker_makes_after_link_directory_substitution_unverifiable(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            def link_then_substitute(source_fd: int, parent_fd: int, destination: str) -> None:
                method = real_link(source_fd, parent_fd, destination)
    ...
    def test_marker_postlink_mutation_replacement_and_extra_hardlink_all_reject(self) -> None:
        if os.name != "posix":
            self.skipTest("fd-addressed publication is a POSIX production contract")
        attacks = ("mutate", "replace", "extra-link")
        for attack_index, attack in enumerate(attacks, start=1):
            with self.subTest(attack=attack), tempfile.TemporaryDirectory() as directory:
                parent_path = Path(directory)
                retained = common.RetainedOutputParent.open_path_source_only(parent_path, "d1" * 32)
                final_name = f"marker-{attack}"
                real_link = common._link_held_unnamed_file_noreplace

                def attack_after_link(source_fd: int, parent_fd: int, destination: str) -> str:
                    method = real_link(source_fd, parent_fd, destination)
                    if attack == "mutate":
                        ...
                        os.rename(destination, destination + ".authentic-aside", src_dir_fd=parent_fd, dst_dir_fd=parent_fd)
                        fd = os.open(
                            destination,
                        ...
                        os.write(fd, b"substituted-marker")
                        os.fsync(fd)
                        os.close(fd)
                    else:
                        os.link(
                            destination, destination + ".extra-hardlink",
                ...
                try:
                    with mock.patch.object(common, "_link_held_unnamed_file_noreplace", side_effect=attack_after_link):
                        with self.assertRaises(common.ContractError):
                            with common.CompletionLastOutput(
                                retained, final_name, f"{40 + attack_index:02x}" * 16,
                            ) as transaction:
                                transaction.write_new("RESULT.bin", b"authentic")
                                transaction.complete(list(transaction.members), "ab" * 32)
                finally:
    ...
    def test_forced_otmpfile_and_both_descriptor_link_branches(self) -> None:
        if os.name != "posix" or not getattr(os, "O_TMPFILE", 0):
            self.skipTest("Linux O_TMPFILE/linkat branch contract")
        with tempfile.TemporaryDirectory() as directory:
            authority_path = Path(directory) / "authority"
            authority_fd = os.open(authority_path, os.O_RDWR | os.O_CREAT | os.O_EXCL, 0o600)
            os.unlink(authority_path)
            with mock.patch.object(common.os, "open", return_value=authority_fd) as patched_open:
                opened, anchor = common._open_held_commit_authority_file(123, "anchor.incomplete")
            self.assertEqual(opened, authority_fd)
            self.assertIsNone(anchor)
            self.assertTrue(patched_open.called)
            os.close(opened)

        with mock.patch.object(common, "_linkat_syscall", return_value=0) as direct:
            method = common._link_held_unnamed_file_noreplace(11, 12, "marker.json")
        self.assertEqual(method, "AT_EMPTY_PATH")
        self.assertEqual(direct.call_count, 1)
        self.assertEqual(direct.call_args.args[1], b"")

        import errno
        with mock.patch.object(common, "_linkat_syscall", side_effect=(errno.ENOENT, 0)) as fallback:
            method = common._link_held_unnamed_file_noreplace(11, 12, "marker.json")
        self.assertEqual(method, "PROC_SELF_FD_AT_SYMLINK_FOLLOW")
        self.assertEqual(fallback.call_count, 2)
        self.assertEqual(fallback.call_args_list[1].args[1], b"/proc/self/fd/11")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                (result / "COMPLETE.json").write_bytes(common.pretty_json(common.seal_record({
                    "schema": "unifilar-wfa-completion-v8",
