# UWFA-SC v8 independent bootstrap ABI
...
- the SHA-256 of an independent v8 source-review receipt whose schema is
  `unifilar-wfa-entropy-census-independent-source-review-v8`;
...
- `container_codec.py`: literal `UWFCV8` build/parse/rebuild/routed-read ledger;
- `stage0_census.py`: all-150 nested experiment and symmetric controls;
- `cupy_backend.py`: exact CUDA path and measured telemetry;
- `result_envelope.py`: retained-parent marker, inode, member-root, and
...
6. Form connected components of the exact stream-owner bipartite graph; require
   at least two disjoint estimand units. Run every source component fold over
   all 150 cells, winner refit, and score one exact literal delta from the
   authenticated current payload/framing layout with no UWFA model to the
   candidate layout containing the selected serialized model exactly once at
   its real aligned placement. Raw model-byte subtraction or equal-model
   cancellation is forbidden. Then perform literal pack/parse/standalone
   decode/re-encode, physical R/F, and fresh routed cold reads.
...
8. Supply a retained, authenticated output-parent descriptor. Create a hidden
   staging directory descriptor-relatively, fsync each member, and write
   `COMPLETE.json` last as a content record. Re-enumerate and no-follow
   reopen/fstat/rehash the exact member set through the retained staging fd.
   Require the staging pathname to name that fd, perform
   `renameat2(RENAME_NOREPLACE)`, then require the final pathname to name the
   same `(st_dev, st_ino)` and repeat the held-fd enumeration/hash.
9. Construct a canonical UTF-8-sorted member table including `COMPLETE.json`
   and the domain-separated directory root. Write the parent commit record to
   an exact held regular-file fd. Prefer `O_TMPFILE`; a filesystem-incompatible
   fallback may use a unique `O_EXCL|O_NOFOLLOW` creation anchor only to obtain
   the fd. Atomically expose the marker from the held fd using
   `linkat(AT_EMPTY_PATH)` or the exact kernel-owned `/proc/self/fd/<n>` source
   with `AT_SYMLINK_FOLLOW`; never rename/link from the mutable anchor name.
   Immediately require both published names to match the retained directory
   and marker fds, remove only a same-inode creation anchor, require marker
   link count one, re-read and rehash marker bytes through the retained fd,
   rehash the directory, and fsync the parent. Do not delete or
   repair a final directory or marker after a later fault.
10. Independent consumers must receive a retained parent descriptor, derive and
   require the parent marker, verify its seal, expected source root, parent,
   marker and final-directory inode bindings, exact member hashes and directory
   root, and matching `COMPLETE.json`. `COMPLETE.json` alone is never complete.
   Recheck both names against held fds after verification. Consumption must
   occur inside the bounded verified-bundle lifetime, using only held fds or
   bytes buffered from them under frozen aggregate/per-member caps; path reopen
   after metadata-only verification is forbidden.
11. A fresh independent result auditor reopens the output and every bound input,
...
The literal result also carries a `uwfa-sc-v8-posterior-diagnostic-handoff`
