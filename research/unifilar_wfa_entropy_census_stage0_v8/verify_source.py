#!/usr/bin/env python3
"""Native verifier for a post-review sealed UWFA-SC v8 source package."""
...
def verify_package(package: Path) -> dict[str, Any]:
    ...
    require(design.get("schema") == "unifilar-wfa-entropy-census-design-v8", "design schema")
    ...
    common = module_from_snapshot("uwfa_verify_common_v8", snapshots["uwfa_common.py"])
    ...
    require(model.startswith(b"UWFAV8\x00\x00"), "v8 serialized model")
    ...
    for token in ("UWFCV8", "UWFDIR4", "UWFREG4", "UWFFRM4", "VERSION = 4", "DIRECTORY_RECORD_BYTES = 256", "REGION_HEADER_BYTES = 256", "FRAME_HEADER_BYTES = 256", "def measure_literal_container_layout", "serialized container differs from shared literal measurement", "def routed_read_expert", "def physical_metrics", "def posterior_diagnostic_handoff", "decoded_selected_decision_triplet_sha256", "modeled_symbol_density", "routed_read_request_aggregates", "frozen_cold_gate_uses_unique_touched_page_bytes_only"):
    ...
    for token in (
        ...
        "def _literal_panel_layout_bits", "def literal_validation_score",
        "def literal_current_baseline_score", "literal_selected_model_aligned_increment_bits",
        "cancel equal model layouts and never subtract raw model bytes",
        "def _dependence_components",
        ...
        require(token in stage_text, f"v8 source gate: {token}")
    ...
    for token in (
        ...
        "UWFA-V8-HELD-DIRECTORY-ROOT", "unifilar-wfa-parent-commit-v8",
    ):
        require(token in common_text, f"descriptor-relative publication contract: {token}")
    require(common_text.count("self._verify_exact_staging_members(") == 5, "held-directory verification at every V8 publication boundary")
    ...
    for token in (
        ...
        "test_outer_model_charge_uses_aligned_literal_increment_not_raw_bytes",
        "test_fold_measurement_equals_actual_serialized_candidate_container",
        ...
        require(token in tests_text, f"V8 hostile regression: {token}")

    return {
        "schema": "unifilar-wfa-source-verification-v8",
